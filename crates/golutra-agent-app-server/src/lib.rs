...
use golutra_agent_core::{Actor, ActorKind, CommandId, SessionId, TaskId, ThreadId, TraceView};
...
pub async fn run_stdio() -> miette::Result<()> {
    ...
    let info = AppServerInfo {
        instance_id: Uuid::now_v7().to_string(),
        pid: std::process::id(),
        base_url: "stdio://golutra-agent-app-server".to_owned(),
        ipc_path: app_server_ipc_path(&paths),
        protocol_versions: ProtocolVersionRange::runtime(),
        started_at: chrono::Utc::now(),
    };
...
fn enforce_artifact_disclosure(
    ...
    if !local_ipc
        && chunk
            .is_some_and(|chunk| chunk.redaction_status == golutra_agent_core::RedactionStatus::Raw)
    {
        return Err(AppError::Disclosure(
...
) -> Result<Json<Vec<golutra_agent_store::ThreadRecord>>, AppError> {
    let transport = state.attached_transport(&headers).await?;
    Ok(Json(
...
) -> Result<Json<Option<golutra_agent_store::ThreadRecord>>, AppError> {
    let transport = state.attached_transport(&headers).await?;
    Ok(Json(
        transport
...
async fn export_thread_rollout(
    ...
    headers: HeaderMap,
    AxumPath(thread_id): AxumPath<String>,
) -> Result<Json<golutra_agent_client::RolloutExport>, AppError> {
    let transport = state.attached_transport(&headers).await?;
    Ok(Json(
        transport
...
) -> Result<Json<golutra_agent_client::ThreadRebindResult>, AppError> {
    let transport = state.attached_transport(&headers).await?;
    Ok(Json(
        transport
    ...
    <script>
      ...
      form.addEventListener("submit", async (event) => {
        ...
        const transportHeaders = {
          ...authHeaders,
          "x-golutra-agent-protocol-version": String(protocolVersion)
        };
        ...
        const response = await fetch(`/events?session_id=${encodeURIComponent(sessionId)}`, {
          headers: {
            ...
            "x-golutra-agent-attachment": attachment.attachment_id
          },
...
mod tests {
    ...
    use golutra_agent_core::{Actor, ActorKind, ArtifactId, CommandId, RedactionStatus};
    use golutra_agent_protocol::{
    ...
    async fn detaching_an_active_task_keeps_the_runtime_available_for_reattach() {
        ...
        let ack = transport
            .execute_operation(RuntimeOperation::SendCommand(
                golutra_agent_protocol::SessionCommand {
                    command_id: CommandId::new(),
                    session_id: Some(first.runtime.default_session_id),
                    kind: golutra_agent_protocol::SessionCommandKind::Prompt,
        ...
        let abort = transport
            .execute_operation(RuntimeOperation::SendCommand(
                golutra_agent_protocol::SessionCommand {
                    command_id: CommandId::new(),
                    session_id: Some(first.runtime.default_session_id),
                    kind: golutra_agent_protocol::SessionCommandKind::Abort,
                    idempotency_key: CommandId::new().to_string(),
                    actor: Actor {
                        kind: ActorKind::Api,
                        id: "active-task-owner".to_owned(),
    ...
    async fn attach_page_is_served() {
        ...
        assert!(body.contains("x-golutra-agent-attachment"));
        assert!(body.contains("runtimeInfo.protocol_versions?.current"));
        assert!(!body.contains("\"x-golutra-agent-protocol-version\": \"2\""));
    }
}
