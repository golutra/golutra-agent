...
use golutra_agent_core::{Actor, ActorKind, CommandId, SessionId, ThreadId};
...
async fn dispatch(
    ...
    let result = match request.method.as_str() {
        "initialize" => Ok(json!({
            "server": "golutra-agent-app-server",
            ...
            "capabilities": [
                ...
                "config.runtime.read", "config.runtime.patch"
            ]
        })),
        "runtime/info" => Ok(json!(state.info())),
        "config/runtime/read" | "config/runtime/patch" => {
            runtime_settings_rpc(state, &request.method, params).await
        }
...
/// 设置仅作用于服务端全局非敏感层，不接受调用者传任意路径或凭据字段。
async fn runtime_settings_rpc(
    state: &AppState,
    method: &str,
    params: Value,
    ...
    let patch = if method == "config/runtime/patch" {
        Some(
            serde_json::from_value::<Patch>(params)
                .map_err(|error| RpcDispatchError::new(-32602, error.to_string()))?,
        )
    } else {
        if params != json!({}) {
            return Err(RpcDispatchError::new(-32602, "read takes no parameters"));
        }
        None
    };
    let home = state.inner.runtime_home.clone().ok_or_else(|| {
        RpcDispatchError::new(-32000, "runtime has no durable configuration home")
    })?;
    tokio::task::spawn_blocking(move || {
        let result =
            golutra_agent_config::ProviderConfigPaths::from_home(home).and_then(
                |paths| match patch {
                    Some(patch) => golutra_agent_config::patch_runtime_settings(
                        &paths,
                        &patch.expected_revision,
                        patch.patch,
                    ),
                    None => golutra_agent_config::read_runtime_settings(&paths),
                },
            );
        result
            .map_err(|error| {
                let code = if matches!(
                    error,
                    golutra_agent_config::ConfigError::VersionConflict { .. }
                ) {
                    -32009
                } else {
                    -32602
                };
                RpcDispatchError::new(code, error.to_string())
            })
            .and_then(|value| {
                serde_json::to_value(value)
                    .map_err(|error| RpcDispatchError::new(-32603, error.to_string()))
            })
    })
    .await
    .map_err(|error| RpcDispatchError::new(-32603, error.to_string()))?
...
async fn session_cursor(
    transport: &golutra_agent_client::EmbeddedTransport,
...
impl RpcDispatchError {
    ...
    fn from_client(error: golutra_agent_client::ClientError) -> Self {
...
mod tests {
    ...
    #[tokio::test]
    async fn runtime_settings_rpc_uses_server_home_and_reports_revision_conflict() {
        let root = tempfile::tempdir().unwrap();
        let (_home, mut state) = test_state();
        Arc::get_mut(&mut state.inner).unwrap().runtime_home = Some(root.path().to_path_buf());
        let first = runtime_settings_rpc(&state, "config/runtime/read", json!({}))
            .await
            .unwrap();
        let requests = ["desktop", "npm"].map(|model| {
            json!({
                "expected_revision": first["revision"], "patch": {"model": model}
            })
        });
        let (left, right) = tokio::join!(
            runtime_settings_rpc(&state, "config/runtime/patch", requests[0].clone()),
            runtime_settings_rpc(&state, "config/runtime/patch", requests[1].clone())
        );
        assert_ne!(left.is_ok(), right.is_ok());
        assert_eq!(left.err().or_else(|| right.err()).unwrap().code, -32009);
        let before = std::fs::read(root.path().join("runtime.json")).unwrap();
        for params in [
            json!({"path":"/outside"}),
            json!({"expected_revision":first["revision"], "patch":{}, "home":"/outside"}),
        ] {
            assert_eq!(
                runtime_settings_rpc(&state, "config/runtime/patch", params)
                    .await
                    .unwrap_err()
                    .code,
                -32602
        ...
        assert_eq!(
            std::fs::read(root.path().join("runtime.json")).unwrap(),
            before
        );
    }
    ...
    fn test_state() -> (tempfile::TempDir, AppState) {
        ...
        let mut state = AppState::new(
            golutra_agent_client::AppServerInfo {
        ...
        .expect("app state");
        // 临时 workspace 不隔离全局数据库；每个 RPC 测试必须绑定自己的 home。
        Arc::get_mut(&mut state.inner).unwrap().runtime_home = Some(home.path().to_path_buf());
        (home, state)
    }
    ...
    async fn insert_test_attachment(state: &AppState, attachment_id: &str) {
        state
            ...
            .insert(
                ...
                golutra_agent_client::EmbeddedTransport::in_memory()
                    .await
                    .expect("transport"),
