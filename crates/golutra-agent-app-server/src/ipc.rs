...
#[cfg(unix)]
use golutra_agent_protocol::{IpcHttpRequest, IpcHttpResponseFrame, MAX_WIRE_MESSAGE_BYTES};
#[cfg(unix)]
use tokio::{
    ...
    for (name, value) in request.headers {
        ...
        if !matches!(
            ...
            "authorization"
                | "content-type"
                | "last-event-id"
                | "x-golutra-agent-attachment"
                | "x-golutra-agent-actor-id"
                | "x-golutra-agent-protocol-version"
        ) {
