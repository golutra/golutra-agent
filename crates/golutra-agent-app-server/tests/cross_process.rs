...
#[cfg(unix)]
use golutra_agent_client::UnixIpcTransport;
...
#[tokio::test]
#[ignore = "set GOLUTRA_AGENT_SOAK_ROUNDS to run the configurable restart soak"]
async fn daemon_restart_soak_preserves_event_invariants() {
    let rounds = env::var("GOLUTRA_AGENT_SOAK_ROUNDS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|rounds| (1..=10_000).contains(rounds))
        .unwrap_or(0);
    if rounds == 0 {
        eprintln!("set GOLUTRA_AGENT_SOAK_ROUNDS=... to run the restart soak");
        return;
    }
    let restart_every = env::var("GOLUTRA_AGENT_SOAK_RESTART_EVERY")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
...
async fn daemon_transport_token_is_private_and_rejects_wrong_credentials() {
    ...
    assert_eq!(
        ...
        golutra_agent_protocol::ProtocolVersionRange::runtime()
    );
    #[cfg(unix)]
    {
...
fn prompt_command(session_id: golutra_agent_core::SessionId, prompt: &str) -> SessionCommand {
    SessionCommand {
...
fn spawn_daemon_at(home: &Path, address: std::net::SocketAddr) -> ChildGuard {
    ChildGuard(
        Command::new(env!("CARGO_BIN_EXE_golutra-agent-app-server"))
            .arg("--addr")
...
async fn wait_for_completion(events: &mut golutra_agent_client::RuntimeEventStream) {
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            let event = events
                .recv()
