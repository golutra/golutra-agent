...
mod compatibility;
mod migrations;
pub use compatibility::{DataCompatibility, DataCompatibilityReport, inspect_data_compatibility};
mod projection;
mod repositories;
...
const ARTIFACT_STORE_IDENTITY_FORMAT: &str = "golutra-agent-artifact-store-v1";
...
pub struct RuntimeStore {
    ...
    _usage_lock: Option<Arc<std::fs::File>>,
}
...
impl RuntimeStore {
    ...
    async fn connect_with_artifact_root_and_journal(
        ...
        let usage_lock = compatibility::open_usage_lock(options.get_filename())?;
        let pool = connect_sqlite_pool_with_retry(options).await?;
        let usage_lock = compatibility::prepare_shared_store(&pool, usage_lock).await?;
    ...
    pub async fn initialize(&self) -> StoreResult<()> {
        compatibility::require_supported(&self.pool).await?;
    ...
    /// 已打开 store 的只读兼容诊断；正常运行期间 usage lease 阻止合作实例迁移。
    pub async fn data_compatibility(&self) -> StoreResult<DataCompatibilityReport> {
        inspect_data_compatibility(&self.pool).await
    }
    ...
    pub async fn load_evidence_by_ids(
        &self,
        evidence_ids: &HashSet<golutra_agent_core::EvidenceId>,
    ) -> StoreResult<Vec<EvidenceRecord>> {
...
fn collect_tool_completion_references(
    ...
    evidence: &mut HashSet<golutra_agent_core::EvidenceId>,
) -> StoreResult<()> {
    match value {
        serde_json::Value::Object(values) => {
...
mod bounded_artifact_tests {
    use chrono::Utc;
    use golutra_agent_core::{ArtifactId, ArtifactRecord, RedactionStatus, SessionId};
