...
use golutra_agent_protocol::{ArtifactReadRequest, RuntimeEventSource, RuntimeEventType};
use serde_json::json;
...
async fn a_new_task_clears_terminal_fields_from_the_previous_task() {
    ...
    store
        ...
        .await
        .expect("assistant");
    event.id = golutra_agent_core::EventId::new();
...
#[tokio::test]
async fn unversioned_threads_are_rejected_without_deduplication() {
    let directory = tempdir().expect("directory");
    let database_url = format!(
        "sqlite://{}",
        directory.path().join("runtime.sqlite").display()
    ...
    assert!(
        RuntimeStore::connect(&database_url)
            .await
            .unwrap_err()
            .to_string()
    ...
    assert_eq!(count, 3);
    let parent: String =
        sqlx::query_scalar("SELECT parent_thread_id FROM threads WHERE thread_id=?")
            .bind(child_thread_id.to_string())
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(parent, older_thread_id.to_string());
...
async fn legacy_projection_without_verification_check_kind_remains_readable() {
    ...
    assert_eq!(
        ...
        golutra_agent_core::VerificationCheckKind::AssistantResponse
    );
}
