...
#[async_trait]
impl golutra_tools::TaskDelegationBackend for ConcurrentChildren {
    async fn delegate(
        &self,
        request: &ToolRequest,
        _: CancellationToken,
    ) -> Result<golutra_tools::TaskDelegationOutput, ToolError> {
    ...
    async fn notifications(
        ...
        Ok(vec![golutra_tools::DelegationNotification {
...
impl BeforeSideEffectRecorder for ChildCheckpoints {
    async fn persist_before_side_effect(
        &self,
        _: &ToolRequest,
        _: &[golutra_tools::FileBeforeImage],
        _: bool,
