...
use super::*;
use golutra_llm::{LlmProvider, ProviderRequest};
use golutra_runtime::AgentReplayContext;
...
fn child_role_contributor() -> golutra_context::ContextContributor {
...
pub(super) async fn capture(
    ...
    let binding = json!({"artifact_id":artifact,"parent_session_id":request.session_id,
        ...
        "history_start":golutra_context::stable_prefix_message_count(&snapshot)});
...
async fn read(host: &RuntimeHost, binding: &Value) -> Result<ProviderRequest, ClientError> {
    ...
    if artifact.session_id != parent
        ...
        || artifact.redaction_status != golutra_core::RedactionStatus::Raw
    {
...
pub(crate) async fn replay(
    host: &RuntimeHost,
    session: SessionId,
    task: golutra_core::TaskId,
    objective: &str,
    payload: &Value,
    provider: &golutra_llm::ConfiguredProvider,
...
mod tests {
    use super::*;
    use golutra_context::{ContextBuilder, ContextContributor, context_snapshot_from_request};
    use golutra_core::{ProviderRequestId, TaskId, TurnId};
    use golutra_llm::{ConfiguredProvider, MockProvider, ProviderRole, ProviderToolCall};
    ...
    async fn explicit_fork_keeps_frozen_complete_pairs_and_rejects_foreign_or_missing_artifacts() {
        ...
        missing[SNAPSHOT_KEY]["artifact_id"] = json!(golutra_core::ArtifactId::new());
        assert!(
