...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GOLUTRA_AGENT_BUILD_BINARY_CHECKSUM");
    ...
    emit(
        "GOLUTRA_AGENT_BUILD_GIT_COMMIT",
        commit.as_deref().unwrap_or(""),
    );
    emit(
        "GOLUTRA_AGENT_BUILD_GIT_DIRTY",
        if dirty { "true" } else { "false" },
    );
    emit("GOLUTRA_AGENT_BUILD_SOURCE_DIGEST", &source_digest);
    emit(
        "GOLUTRA_AGENT_BUILD_CARGO_LOCK_DIGEST",
        cargo_lock_digest.as_deref().unwrap_or(""),
    );
    emit("GOLUTRA_AGENT_BUILD_RUSTC_VERSION", &rustc_version);
    emit(
        "GOLUTRA_AGENT_BUILD_TARGET",
        &env::var("TARGET").unwrap_or_else(|_| "unknown".into()),
    );
    emit(
        "GOLUTRA_AGENT_BUILD_PROFILE",
        &env::var("PROFILE").unwrap_or_else(|_| "unknown".into()),
    );
    emit("GOLUTRA_AGENT_BUILD_FEATURES", &features.join(","));
    emit(
        "GOLUTRA_AGENT_BUILD_BINARY_CHECKSUM",
        &env::var("GOLUTRA_AGENT_BUILD_BINARY_CHECKSUM").unwrap_or_default(),
    );
}
