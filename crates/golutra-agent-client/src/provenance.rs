...
fn capture_build_provenance() -> BuildProvenance {
    BuildProvenance {
        ...
        git_commit: non_empty(option_env!("GOLUTRA_AGENT_BUILD_GIT_COMMIT")),
        dirty: option_env!("GOLUTRA_AGENT_BUILD_GIT_DIRTY") == Some("true"),
        source_digest: non_empty(option_env!("GOLUTRA_AGENT_BUILD_SOURCE_DIGEST")),
        cargo_lock_digest: non_empty(option_env!("GOLUTRA_AGENT_BUILD_CARGO_LOCK_DIGEST")),
        target: option_env!("GOLUTRA_AGENT_BUILD_TARGET")
            .unwrap_or("unknown")
            .to_owned(),
        profile: option_env!("GOLUTRA_AGENT_BUILD_PROFILE")
            .unwrap_or("unknown")
            .to_owned(),
        features: option_env!("GOLUTRA_AGENT_BUILD_FEATURES")
            .unwrap_or_default()
            .split(',')
            .filter(|feature| !feature.is_empty())
            .map(str::to_owned)
            .collect(),
        rustc_version: option_env!("GOLUTRA_AGENT_BUILD_RUSTC_VERSION")
            .unwrap_or("unknown")
            .to_owned(),
        ...
        binary_checksum: binary_checksum_from_metadata(option_env!(
            "GOLUTRA_AGENT_BUILD_BINARY_CHECKSUM"
        )),
    }
}
