...
pub(super) fn task_outcome_from_verification(
    ...
    status: TaskStatus,
    verification: &golutra_agent_core::VerificationRecord,
) -> golutra_agent_core::TaskOutcome {
...
pub(super) fn task_outcome_with_external_verification(
    ...
    let external_verification = if defer_external_verification {
        golutra_agent_core::ExternalVerificationStatus::Pending
    } else {
        golutra_agent_core::ExternalVerificationStatus::NotRequested
    };
    let outcome = if defer_external_verification && candidate_ready_for_external_verification {
        golutra_agent_core::TaskOutcome::candidate_ready(verification)
    } else {
        golutra_agent_core::TaskOutcome::from_verification(status, verification)
    };
...
impl RuntimeHost {
    ...
    pub(super) async fn persist_checkpoint_before_side_effect(
        ...
        let checkpoint_result = run_blocking(move || {
            ...
            Ok::<_, golutra_agent_runtime::CheckpointError>((
                checkpoint,
                checkpoint_before_images,
                excluded_count,
            ))
    ...
    pub(super) async fn record_tool_report(
        &self,
        task: &HostedAgentTask,
        report: &golutra_agent_tools::ToolExecutionReport,
    ) -> Result<(), ClientError> {
        ...
        let diff_artifact = change_facts.diff_artifact.as_ref().map(|diff| {
            let (content, redaction_status) =
                golutra_agent_tools::redact_sensitive_text(&diff.content);
    ...
    pub(super) async fn finish_lane(
        ...
        self.finish_lane_with_outcome(
            task,
            status,
            golutra_agent_core::TaskOutcome::from_status(
                status,
                golutra_agent_core::VerificationResult::Unknown,
            ),
        )
        .await
    }
    ...
    pub(super) async fn finish_lane_with_outcome(
        ...
        task: &HostedAgentTask,
        status: TaskStatus,
        outcome: golutra_agent_core::TaskOutcome,
    ) -> Result<(), ClientError> {
        ...
        if awaiting_external {
            let candidate_ready =
                outcome.execution == golutra_agent_core::ExecutionOutcome::CandidateReady;
            self.record_event(agent_event(
                self.next_sequence_no(),
                task,
    ...
    pub(super) async fn record_failed_verification(
        ...
        objective: &str,
        reason: &str,
    ) -> Result<golutra_agent_core::VerificationRecord, ClientError> {
