...
use golutra_agent_protocol::{AgentTurnResult, RuntimeEvent, SessionWindowRequest, TaskTracePage};
use golutra_agent_store::RuntimeStore;
...
impl<'a> RunBundleExporter<'a> {
    ...
    async fn export_with_mode(
        ...
        let manifest = RunBundleManifest {
            format: "golutra-agent-run-bundle".to_owned(),
...
fn write_observations(
    ...
    let temporary = tempfile::Builder::new()
        .prefix(".golutra-agent-observations-")
        .tempdir_in(run_root)
    ...
    let manifest = ObservationBundleManifest {
        format: "golutra-agent-runtime-observation".to_owned(),
...
pub(crate) async fn validate_persisted_run_store(
    ...
    if manifest.observations.format != "golutra-agent-runtime-observation"
...
fn recover_stale_directory_swap(destination: &Path) -> Result<(), ClientError> {
    ...
    let temporary_prefix = format!(".golutra-agent-{base}-");
    ...
    for session in &snapshot.sessions {
        for task in &session.tasks {
            if let Some(evaluation) = task.trace.evaluation.external_evaluations.last()
                && evaluation.verdict == golutra_agent_eval::EvaluationVerdict::Fail
                && evaluation
                    .terminal_cause
                    .as_ref()
                    ...
                    final_message: task
                        ...
                        .find(|candidate| {
                            candidate.event_type
                                == golutra_agent_protocol::RuntimeEventType::AssistantMessage
                        })
                        .and_then(|candidate| {
                            candidate
                                .payload
...
mod tests {
    ...
    fn atomic_file_publication_hides_the_destination_until_staging_is_complete() {
        ...
        let bytes = br#"{"format":"golutra-agent-run-bundle"}"#;
