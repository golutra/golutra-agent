...
use golutra_agent_core::{ArtifactId, ArtifactRecord, RedactionStatus, SessionId, TaskId};
use golutra_agent_protocol::{
    ArtifactReadRequest, SessionRangeDirection, SessionRangeSpec, SessionSummary,
    SessionWindowRequest,
};
use golutra_agent_store::MAX_ARTIFACT_READ_BYTES;
...
impl<'a> DebugExportCoordinator<'a> {
    ...
    pub(crate) async fn export_snapshot(
        ...
        let manifest = DebugExportManifest {
            format: "golutra-agent-debug-export".to_owned(),
