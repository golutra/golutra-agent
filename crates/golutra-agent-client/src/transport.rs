...
use golutra_agent_protocol::{decode_event, encode_command_value_for_protocol};
...
mod lifecycle_tests {
    ...
    use golutra_agent_core::{SessionId, ThreadId, WorkspaceId};
    use golutra_agent_protocol::{EventFilter, ProtocolVersionRange};
    ...
    async fn ipc_subscription_rejects_a_closed_transport_before_spawning() {
        let transport = super::UnixIpcTransport {
            socket_path: PathBuf::from("/tmp/golutra-agent-test.sock"),
            server_info: test_server_info(),
            protocol_version: crate::RUNTIME_PROTOCOL_VERSION,
            info: test_runtime_info("/workspace"),
            cwd: PathBuf::from("/workspace"),
    ...
    async fn failed_ipc_close_keeps_the_attachment_id_for_a_retry() {
        let transport = super::UnixIpcTransport {
            socket_path: PathBuf::from("/tmp/golutra-agent-missing-close.sock"),
            server_info: test_server_info(),
            protocol_version: crate::RUNTIME_PROTOCOL_VERSION,
            info: test_runtime_info("/workspace"),
            cwd: PathBuf::from("/workspace"),
