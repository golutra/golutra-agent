...
use golutra_agent_context::{ContextBudgetPolicy, ContextBuilder};
use golutra_agent_llm::{
    ConfiguredProvider, MockProvider, ProviderError, ProviderProtocol, protocol_capabilities,
};
use golutra_agent_runtime::ProviderSessionPolicy;
...
fn task_mock_provider(payload: &Value, objective: &str) -> (MockProvider, bool, bool) {
    ...
    if lower.contains("sleep") {
        ...
        let marker = interruptible
            .then(|| std::env::var_os("GOLUTRA_AGENT_TEST_INTERRUPTIBLE_SHELL_MARKER"))
            .flatten()
...
fn provider_route_cache_key(
    ...
    hasher.update(b"golutra-agent-provider-route-v1\0");
...
const ROUTE_ENV_KEYS: &[&str] = &[
    "GOLUTRA_AGENT_PROVIDER_MODE",
    "GOLUTRA_AGENT_PROVIDER_PROTOCOL",
    ...
    "GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES",
    "GOLUTRA_AGENT_PROVIDER_ROUTE_ID",
    "GOLUTRA_AGENT_PROVIDER_AUTH_PROVIDER",
    "GOLUTRA_AGENT_PROVIDER_FALLBACK_PROTOCOL",
    "GOLUTRA_AGENT_PROVIDER_STREAM_MAX_RETRIES",
    "GOLUTRA_AGENT_PROVIDER_REQUEST_MAX_RETRIES",
    "GOLUTRA_AGENT_PROVIDER_STREAM_IDLE_TIMEOUT_MS",
    "GOLUTRA_AGENT_PROVIDER_REQUEST_TIMEOUT_MS",
    "GOLUTRA_AGENT_PROVIDER_TRANSPORT_FALLBACK",
    "OPENAI_API_KEY",
    "OPENAI_MODEL",
    "OPENAI_BASE_URL",
    "ANTHROPIC_API_KEY",
...
fn provider_runtime_env_for_payload(
    ...
    if let Some(model_id) = model_id {
        environment = environment.with_runtime_override("GOLUTRA_AGENT_PROVIDER_MODEL", model_id);
    }
    if let Some(generation_config) = generation_config {
        environment = environment.with_runtime_override(
            "GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG",
            generation_config,
        );
    }
...
fn generation_config_override(payload: &Value) -> Result<Option<String>, ProviderError> {
    payload
        ...
        .map(|value| {
            serde_json::from_value::<golutra_agent_llm::ProviderGenerationConfig>(value.clone())
                .map_err(|error| ProviderError::NotConfigured {
...
pub(crate) fn configured_provider_plan(
    provider_env: Option<&golutra_agent_config::ProviderRuntimeEnv>,
    mock: MockProvider,
    touched_code: bool,
    ...
    let fallback_provider = provider_env
        .and_then(|environment| environment.get("GOLUTRA_AGENT_PROVIDER_FALLBACK_PROTOCOL"))
        .or_else(|| std::env::var("GOLUTRA_AGENT_PROVIDER_FALLBACK_PROTOCOL").ok())
...
fn provider_session_policy_from_env(
    ...
    if let Some(value) =
        provider_runtime_value(provider_env, "GOLUTRA_AGENT_PROVIDER_STREAM_MAX_RETRIES")
    {
        policy.max_stream_retries = parse_provider_u32("stream max retries", &value)?;
    }
    if let Some(value) =
        provider_runtime_value(provider_env, "GOLUTRA_AGENT_PROVIDER_REQUEST_MAX_RETRIES")
    {
        policy.max_request_retries = parse_provider_u32("request max retries", &value)?;
    }
    if let Some(value) = provider_runtime_value(
        provider_env,
        "GOLUTRA_AGENT_PROVIDER_STREAM_IDLE_TIMEOUT_MS",
    ) {
    ...
    if let Some(value) =
        provider_runtime_value(provider_env, "GOLUTRA_AGENT_PROVIDER_REQUEST_TIMEOUT_MS")
    {
        policy.request_timeout =
            Duration::from_millis(parse_provider_u64("request timeout", &value)?);
    }
    if let Some(value) =
        provider_runtime_value(provider_env, "GOLUTRA_AGENT_PROVIDER_TRANSPORT_FALLBACK")
    {
...
fn context_builder_from_provider_env(
    ...
    let protocol = provider_env
        .and_then(|environment| environment.get("GOLUTRA_AGENT_PROVIDER_PROTOCOL"))
        .and_then(|value| ProviderProtocol::from_config_value(&value))
        .or_else(|| {
            std::env::var("GOLUTRA_AGENT_PROVIDER_PROTOCOL")
                .ok()
                .and_then(|value| ProviderProtocol::from_config_value(&value))
        });
    let declared_capabilities = protocol.map(protocol_capabilities);
    let Some(raw_config) = provider_env
        .and_then(|environment| environment.get("GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG"))
        .or_else(|| std::env::var("GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG").ok())
        .filter(|value| !value.trim().is_empty())
    else {
    ...
    let config: golutra_agent_llm::ProviderGenerationConfig = serde_json::from_str(&raw_config)
        .map_err(|error| ProviderError::NotConfigured {
...
mod tests {
    ...
    use golutra_agent_config::{ProviderProfile, ProviderSettings};
    use golutra_agent_llm::{LlmProvider, ProviderReasoningEffort};
    ...
    fn task_provider_overrides_are_ephemeral_and_validated() {
        ...
        assert_eq!(
            environment.get("GOLUTRA_AGENT_PROVIDER_MODEL").as_deref(),
            Some("session-model")
        );
        let generation = environment
            .get("GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG")
            .and_then(|value| {
                serde_json::from_str::<golutra_agent_llm::ProviderGenerationConfig>(&value).ok()
            })
