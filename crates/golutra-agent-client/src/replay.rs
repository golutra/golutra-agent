...
use golutra_agent_eval::{ReplayCapsule, ReplayExecution, ReplayExecutionStatus};
use golutra_agent_llm::{LlmProvider, ProviderError, ProviderRequest, ProviderResponse};
use golutra_agent_protocol::{RuntimeEvent, RuntimeEventSource, RuntimeEventType};
...
use golutra_agent_tools::{ToolError, ToolReplayBackend, ToolRequest, ToolRuntime};
...
const MAX_REPLAY_ARTIFACT_BYTES: u64 = golutra_agent_tools::MAX_TOOL_ARTIFACT_CONTENT_BYTES as u64;
...
fn validate_replay_capsule_identity(
    ...
    let source_run_id = golutra_agent_core::RunId::from(capsule.source_task_id);
...
fn replay_context_builder(
    snapshot: Option<&golutra_agent_core::ContextSnapshot>,
) -> ContextBuilder {
...
mod tests {
    ...
    fn replay_turn_plan_reconstructs_updated_turns_and_profile_steers() {
        ...
        assert_eq!(
            ...
            Some(golutra_agent_protocol::AgentExecutionMode::Strict)
        );
        assert_eq!(
    ...
    async fn replay_provider_injects_pending_turns_at_the_recorded_response_boundary() {
        ...
        let pending = ConfiguredPendingAgentTurn {
            ...
            execution: golutra_agent_runtime::PendingTurnExecutionOptions {
    ...
    fn replay_restores_the_recorded_task_contract_instead_of_using_a_default() {
        ...
        assert_eq!(
            ...
            golutra_agent_core::WorkspaceChangeRequirement::Forbidden
        );
        assert_eq!(
    ...
    fn replay_reconstructs_legacy_change_contract_without_a_fake_delivery_path() {
        ...
        assert_eq!(
            contract.workspace_change,
            golutra_agent_core::WorkspaceChangeRequirement::Required
        );
