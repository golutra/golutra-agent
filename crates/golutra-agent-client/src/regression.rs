//! Baseline/candidate 隔离执行编排；`golutra-agent-eval` 只保存事实和执行纯比较。
...
use golutra_agent_eval::{FrozenCandidatePatch, RegressionResult};
use golutra_agent_protocol::{SessionCommand, SessionCommandKind, TaskTraceRequest};
...
impl RuntimeHost {
    ...
    pub(crate) fn automatically_process_improvement_candidate<'a>(
        ...
        async move {
            ...
            if candidate.kind != golutra_agent_eval::AutomationCandidateKind::RuntimeChange {
                return Ok(());
            }
    ...
    async fn record_automatic_improvement_events(
        ...
        decision: Option<&golutra_agent_eval::PromotionDecision>,
    ) -> Result<(), ClientError> {
        let events = self
            .storage
            .repositories
...
fn included_snapshot_entry(entry: &DirEntry) -> bool {
    ...
    !matches!(
        ...
        Some(".git" | ".golutra" | ".golutra-agent" | "target" | "node_modules")
    )
}

fn evaluation_case_partition(case: &golutra_agent_eval::EvaluationCase) -> EvaluationPartitionKind {
...
fn sealed_candidate_path_reason(relative: &Path) -> Option<&'static str> {
    ...
    if matches!(
        first,
        Some(".git" | ".golutra" | ".golutra-agent" | ".github" | "target" | "node_modules")
    ) {
...
mod tests {
    ...
    fn candidate_files_reject_control_plane_and_runtime_metadata_paths() {
        ...
        for path in [
            ".git/config",
            ".golutra-agent/runtime.json",
            ".github/workflows/evaluation.yml",
            "crates/golutra-agent-eval/src/runner.rs",
            "crates/golutra-agent-verify/src/lib.rs",
            "crates/golutra-agent-policy/src/lib.rs",
            "crates/golutra-agent-sandbox/src/lib.rs",
