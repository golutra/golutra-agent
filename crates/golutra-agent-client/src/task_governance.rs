...
pub(super) fn trajectory_failure_family(payload: &Value, status: &str) -> String {
    ...
    if let Some(family) = golutra_agent_core::semantic_tool_failure_family(tool_name, facts) {
        return family;
    }
    let summary = payload
        .get("summary")
...
impl RuntimeHost {
    ...
    async fn build_task_evaluation_input(
        ...
        let evaluation_input = TaskEvaluationInput {
            ...
            token_usage,
            provider_config_ref,
            runtime_config_ref: format!("golutra-agent-runtime:{}", env!("CARGO_PKG_VERSION")),
    ...
    async fn quarantine_task_memory(
        ...
        final_message: &str,
        tool_facts: &str,
        evidence_refs: Vec<golutra_agent_core::EvidenceId>,
    ) -> Result<(), ClientError> {
