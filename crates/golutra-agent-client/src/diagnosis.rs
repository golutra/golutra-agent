...
use golutra_agent_core::{
    ArtifactId, CausalRelation, EventId, EvidenceId, ProviderRequestId, TaskId,
};
...
fn classify_failure(
        ...
        && repeated_failure_is_single_round(events, trigger_index, trigger)
    {
        return (
            ...
            vec![
                "cargo test -p golutra-agent-runtime duplicate_failures_in_one_provider_round_can_recover_on_the_next_round"
                    .to_owned(),
            ],
            98,
        );
        ...
        && let Some((family, failures)) = dominant_failure_family(events, trigger_index)
    {
        return (
            ...
            "retain failures across unrelated diagnostic successes and inject a structured strategy correction"
                .to_owned(),
            vec![CodeTargetRef {
                crate_name: "golutra-agent-runtime".to_owned(),
                module_path: "failure_family".to_owned(),
                symbol: Some("FailureFamilyLedger".to_owned()),
                source_path: Some("crates/golutra-agent-runtime/src/lib.rs".to_owned()),
                source_digest,
                owner: "runtime".to_owned(),
            }],
            vec!["cargo test -p golutra-agent-runtime semantic_failure_families".to_owned()],
            96,
        );
    }
    if let Some(trigger_kind) = trigger_kind {
        return (
            ...
            vec!["cargo test -p golutra-agent-runtime".to_owned()],
            90,
        );
    }
    if trigger.event_type == RuntimeEventType::ToolCompleted {
        ...
        return (
            ...
            vec![CodeTargetRef {
                crate_name: "golutra-agent-tools".to_owned(),
                module_path: tool_name.to_owned(),
                symbol: None,
                source_path: Some("crates/golutra-agent-tools/src".to_owned()),
                source_digest,
                owner: "tools".to_owned(),
            }],
            vec!["cargo test -p golutra-agent-tools".to_owned()],
            82,
        );
    }
    if trigger.event_type == RuntimeEventType::ProviderFailed {
        ...
        return (
            ...
            "replay the request with the same provider contract and verify retry/fallback handling"
                .to_owned(),
            vec![CodeTargetRef {
                crate_name: "golutra-agent-llm".to_owned(),
                module_path: provider.to_owned(),
                symbol: None,
                source_path: Some("crates/golutra-agent-llm/src".to_owned()),
                source_digest,
                owner: "provider".to_owned(),
            }],
            vec!["cargo test -p golutra-agent-llm".to_owned()],
            85,
        );
    }
        ...
        if malformed_expectation || placeholder_mismatch {
            ...
            return (
                ...
                vec![CodeTargetRef {
                    ...
                    source_path: Some(
                        "crates/golutra-agent-runtime/src/objective_evidence.rs".to_owned(),
                    ),
                    source_digest,
                    owner: "runtime".to_owned(),
                }],
                vec![
                    "cargo test -p golutra-agent-runtime objective_evidence::tests".to_owned(),
                ],
                97,
            );
        }
...
mod tests {
    use chrono::Utc;
    use golutra_agent_core::{CausalContext, EvidenceId};
    ...
    fn event(
        ...
        RuntimeEvent {
            ...
            id: EventId::new(),
            sequence_no,
            session_id: golutra_agent_core::SessionId::new(),
            turn_id: None,
    ...
    fn malformed_delivery_path_failure_targets_the_extractor() {
        ...
        assert_eq!(
            ...
            Some("crates/golutra-agent-runtime/src/objective_evidence.rs")
        );
    ...
    fn diagnostic_slice_prioritizes_causal_history_and_publishes_trace_pages() {
        ...
        for sequence_no in 2..700 {
            ...
            unrelated.causal_links.push(golutra_agent_core::CausalLink {
        ...
        trigger.causal_links.push(golutra_agent_core::CausalLink {
            event_id: previous_id,
            relation: CausalRelation::Parent,
        });
        trigger.causal_links.push(golutra_agent_core::CausalLink {
