...
use golutra_agent_context::{
    ContextContributor, estimate_tokens, parse_compaction_summary_envelope,
};
use golutra_agent_core::{
    EventId, SessionId, TaskContract, TaskId, TurnId, VerificationRequirement,
};
use golutra_agent_evolution::SkillManifest;
use golutra_agent_llm::ProviderRole;
use golutra_agent_memory::RetrievedMemory;
use golutra_agent_protocol::{RuntimeEvent, RuntimeEventType};
use golutra_agent_tools::model_visible_tool_result_with_limit;
...
fn historical_tool_result_content(event: &RuntimeEvent) -> Option<String> {
    ...
    let rendered = envelope_value
        .and_then(|value| {
            serde_json::from_value::<golutra_agent_core::ToolResultEnvelope>(value.clone())
                .ok()
                .map(|envelope| {
