...
use golutra_agent_core::{EvidenceId, PostTaskJobStatus, SessionId, TaskId};
...
use golutra_agent_memory::{MemoryError, MemoryRecord, MemoryStore, propose_project_memory};
use golutra_agent_protocol::{ContextProjection, EvaluationProjection, RuntimeEventType};
use golutra_agent_store::RuntimeRepositories;
    ...
    pub(crate) async fn candidate_source_task_id(
        ...
        let task_id = run_blocking(move || {
            ...
            state
                ...
                .ok_or(golutra_agent_eval::EvaluationError::CandidateNotFound(
                    candidate_id,
                ))
        })
        .await??;
