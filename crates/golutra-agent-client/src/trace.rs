...
use golutra_agent_protocol::{ArtifactChunk, ArtifactReadRequest, TaskTracePage, TaskTraceRequest};
use golutra_agent_store::{MAX_ARTIFACT_READ_BYTES, StoreError};
...
pub(crate) async fn read_task_trace(
    ...
    let mut verification: Option<golutra_agent_core::VerificationRecord> = None;
    for event in &events {
    ...
    let mut evidence = repositories
        ...
        .fold(Vec::new(), |mut records, record| {
            if !records
                .iter()
                .any(|existing: &golutra_agent_core::EvidenceRecord| {
