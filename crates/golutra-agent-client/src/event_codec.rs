...
use golutra_agent_protocol::{EventFilter, RuntimeEvent, RuntimeEventSource, RuntimeEventType};
use golutra_agent_runtime::{
    AgentLoopTraceEvent, PendingAgentTurn, PendingTurnExecutionOptions, RuntimeObservation,
};
use golutra_agent_store::ThreadRecord;
use golutra_agent_tools::redact_sensitive_text;
        ...
        AgentLoopTraceEvent::UserStep(step) => Some((
            ...
            json!({
                "summary": match &step.kind {
                    golutra_agent_core::UserStepKind::AssistantText { text } => compact_event_summary(text),
                    golutra_agent_core::UserStepKind::ToolBatch { summary, .. } => summary.clone(),
                },
                "step": step,
            }),
        )),
...
fn context_message_wire_prefix_digest_for_count(
    ...
    digest.update(b"golutra-agent-context-prefix-v1\0");
...
pub(crate) fn with_command_payload(
    ...
    command_id: golutra_agent_core::CommandId,
    payload: Value,
) -> RuntimeEvent {
    event.payload = json!({
        "summary": event
...
mod tests {
    ...
    fn legacy_pending_turn_recovery_rebuilds_its_task_contract() {
        ...
        assert_eq!(
            ...
            Some(golutra_agent_protocol::AgentToolProfile::Coding)
        );
    }
    ...
    fn steering_recovery_inherits_the_active_tool_profile_unless_explicit() {
        for (payload, expected) in [
            ...
            (
                ...
                Some(golutra_agent_protocol::AgentToolProfile::Full),
            ),
        ] {
            let mut event = host_event(
                7,
    ...
    fn context_cache_diagnostics_are_wire_complete_without_sensitive_content() {
        ...
        let turn_id = TurnId::new();
        let messages = vec![
            golutra_agent_llm::ProviderMessage {
            ...
            golutra_agent_llm::ProviderMessage {
                role: golutra_agent_llm::ProviderRole::Assistant,
                content: "calling a tool".to_owned(),
                tool_call_id: None,
                tool_name: None,
                tool_calls: vec![golutra_agent_llm::ProviderToolCall {
        ...
        request.cache_scope = Some(golutra_agent_llm::PromptCacheScope::session(
            session_id, None,
        ));
    ...
    fn cache_diagnostics_prove_append_only_message_prefix_and_tool_invalidation() {
        ...
        second_messages.push(golutra_agent_llm::ProviderMessage {
            role: golutra_agent_llm::ProviderRole::Assistant,
        ...
        let tool = golutra_agent_core::ToolContract {
            tool_name: "read_file".to_owned(),
            input_schema: json!({
                "type": "object",
        ...
        first_request.cache_scope = Some(golutra_agent_llm::PromptCacheScope::session(
            session_id, None,
        ));
    ...
    fn provider_completed_projects_only_bounded_transport_diagnostics() {
        let response = ProviderResponse {
            ...
            message: None,
            tool_calls: Vec::new(),
            usage: golutra_agent_llm::ProviderUsage {
                input_tokens: Some(10),
                output_tokens: Some(2),
                reasoning_tokens: None,
                cached_input_tokens: Some(8),
                total_tokens: Some(12),
                usage_source: golutra_agent_core::UsageSource::Provider,
                raw: json!({"input_tokens": 10}),
            },
            finish_reason: golutra_agent_llm::ProviderFinishReason::Stop,
            raw_metadata: json!({
