...
use golutra_agent_auth::{AuthService, CredentialRef, MemorySecretStore, SecretKind, SecretStore};
...
fn open_wire_mode_checks_completion_without_requiring_unrequested_changes() {
    ...
    assert_eq!(
        contract.verification,
        golutra_agent_core::VerificationRequirement::BestEffort
    );
}
...
fn observation_catalog_classifies_loop_facts_before_persistence() {
    let required = observation_descriptor(&RuntimeObservation::ToolStarted {
        ...
        recovery_policy: golutra_agent_core::ToolRecoveryPolicy::for_side_effect(
            golutra_agent_core::SideEffectType::None,
        ),
    });
    ...
    let diagnostic = observation_descriptor(&RuntimeObservation::ProviderStreamed {
        ...
        event: golutra_agent_llm::ProviderStreamEvent::TextDelta {
            text: "delta".to_owned(),
        },
    });
...
async fn hosted_execution_channel_exposes_one_active_surface() {
    ...
    execution.set_active_execution_surface(
        Some(golutra_agent_protocol::AgentExecutionMode::Open),
        golutra_agent_protocol::AgentToolProfile::Coding,
    );
    assert_eq!(
        execution.active_execution_surface(),
        golutra_agent_runtime::ActiveExecutionSurface {
            execution_mode: Some(golutra_agent_protocol::AgentExecutionMode::Open),
            tool_profile: golutra_agent_protocol::AgentToolProfile::Coding,
        }
    );
}
...
async fn delegated_task_inherits_overrides_and_archives_an_isolated_child() {
    ...
    let (inherited, concurrent_retry) = tokio::join!(
        golutra_agent_tools::TaskDelegationBackend::delegate(
            &backend,
            &inherited_request,
            CancellationToken::new(),
        ),
    ...
    assert_eq!(inherited.status, golutra_agent_core::ToolResultStatus::Ok);
    ...
    let retried = golutra_agent_tools::TaskDelegationBackend::delegate(
        &backend,
        &inherited_request,
        CancellationToken::new(),
    )
    ...
    let changed_arguments = golutra_agent_tools::TaskDelegationBackend::delegate(
        &backend,
        &ToolRequest {
    ...
    let overridden = golutra_agent_tools::TaskDelegationBackend::delegate(
        &backend,
        &ToolRequest {
            tool_call_id: ToolCallId::new(),
...
async fn assert_delegation_lifecycle(
    ...
    backend: &dyn golutra_agent_tools::TaskDelegationBackend,
    parent: SessionId,
) {
    ...
    assert_eq!(started.status, golutra_agent_core::ToolResultStatus::Ok);
...
async fn assert_delegation_input_and_cancel(
    ...
    assert_eq!(
        stopped.status,
        golutra_agent_core::ToolResultStatus::Ok,
        "{stopped:?}"
    );
...
async fn cancelled_delegation_does_not_create_a_child_session() {
    ...
    let output = golutra_agent_tools::TaskDelegationBackend::delegate(
        &backend,
        &ToolRequest {
            tool_call_id: ToolCallId::new(),
...
async fn cwd_transport_ignores_project_golutra_directory() {
    ...
    symlink(outside.path(), workspace.path().join(".golutra-agent")).expect("symlink");
...
async fn failed_task_blocks_unfrozen_regression_without_polluting_memory() {
    ...
    assert!(trace.evaluation.regressions.iter().any(|regression| {
        regression.candidate_id == runtime_candidate_id
            && regression.verdict == golutra_agent_eval::RegressionVerdict::NeedsReview
    }));
    assert!(
        !trace
            .evaluation
...
async fn explicit_home_transport_reuses_latest_session_without_process_env() {
    ...
    assert!(!workspace.path().join(".golutra-agent").exists());
    #[cfg(unix)]
    {
...
async fn prompt_cache_scope_uses_only_trusted_thread_lineage() {
    ...
    assert_eq!(
        session_scope.kind(),
        golutra_agent_llm::PromptCacheScopeKind::Session
    );
    ...
    assert_eq!(
        fork_scope.kind(),
        golutra_agent_llm::PromptCacheScopeKind::Fork
    );
    ...
    assert_eq!(
        ...
        golutra_agent_llm::PromptCacheScopeKind::Subagent
    );
...
async fn optional_context_cannot_displace_recent_history_in_a_small_budget() {
    ...
    let candidate =
        golutra_agent_memory::propose_project_memory(memory_task_id, vec![EvidenceId::new()]);
    let memory = host
        .storage
        .memory_store
        .quarantine(
...
async fn persisted_ephemeral_runtime_retains_isolated_state_and_full_run_bundle() {
    ...
    let mut handle = thread
        .start_turn(
            ...
            golutra_agent_protocol::AgentTurnOptions {
                defer_external_verification: true,
                ..golutra_agent_protocol::AgentTurnOptions::default()
            },
        )
        .await
        .expect("turn");
    ...
    assert_eq!(
        ...
        Some(golutra_agent_core::ExecutionOutcome::CandidateReady)
    );
    assert_eq!(
        result
            .outcome
        ...
        Some(golutra_agent_core::ExternalVerificationStatus::Pending)
    );
    ...
    assert_eq!(run_manifest.format, "golutra-agent-run-bundle");
    ...
    let trace: golutra_agent_protocol::TaskTracePage = serde_json::from_slice(
    ...
    let mut external = golutra_agent_eval::ExternalEvaluationRecord {
        ...
        verdict: golutra_agent_eval::EvaluationVerdict::Pass,
        score: Some(1.0),
        score_max: Some(1.0),
        assertions: vec![golutra_agent_eval::ExternalEvaluationAssertion {
        ...
        partition: golutra_agent_eval::EvaluationPartitionKind::Source,
        seed: None,
        ...
        trust: golutra_agent_eval::ExternalEvaluationTrust::OwnerLocal,
        attestation: None,
        ingested_at: chrono::Utc::now(),
    };
    external.result_digest = golutra_agent_eval::external_evaluation_result_digest(&external);
    ...
    let refreshed_trace: golutra_agent_protocol::TaskTracePage = serde_json::from_slice(
        &fs::read(observation_root.join(format!(
            "sessions/{}/tasks/{task_id}/trace.json",
            result.session_id
        )))
...
async fn persisted_interruption_accepts_a_prefix_bound_owner_evaluation() {
    ...
    let evaluation_for = |trace: &golutra_agent_protocol::TaskTracePage| {
        let mut record = golutra_agent_eval::ExternalEvaluationRecord {
            ...
            verdict: golutra_agent_eval::EvaluationVerdict::Fail,
            score: Some(0.0),
            score_max: Some(1.0),
            assertions: Vec::new(),
            phases: Vec::new(),
        ...
        record.result_digest = golutra_agent_eval::external_evaluation_result_digest(&record);
        record
    };
    ...
    interrupted_manifest.terminal_outcome = RunBundleTerminalOutcome::Result {
        result: golutra_agent_protocol::AgentTurnResult {
            thread_id,
            session_id,
            task_id: Some(task_id),
            turn_id: Some(turn_id),
    ...
    untrusted.trust = golutra_agent_eval::ExternalEvaluationTrust::UntrustedLocal;
    untrusted.result_digest = golutra_agent_eval::external_evaluation_result_digest(&untrusted);
...
fn recovered_governor_does_not_recharge_usage_accounted_by_transfer() {
    ...
    let task_id = TaskId::new();
    let turn_id = TurnId::new();
    let response_id = golutra_agent_core::ProviderResponseId::new();
    ...
    let usage = TokenUsageRecord {
        ...
        request_event_id: golutra_agent_core::ProviderRequestId::new(),
        response_event_id: response_id,
        input_tokens: Some(10),
        output_tokens: Some(5),
        reasoning_tokens: None,
        estimated_cost: Some(0.001),
        budget_snapshot_ref: golutra_agent_core::TokenBudgetSnapshotId::new(),
...
fn mock_write_file_args_prefers_payload_over_prompt() {
    ...
    let mut contract = golutra_agent_core::TaskContract::default();
    ...
    assert_eq!(
        ...
        vec![golutra_agent_core::RequiredFileContent {
            path: "explicit.txt".to_owned(),
            content: "explicit".to_owned(),
        }]
    );
...
fn legacy_contract_distinguishes_inputs_conversions_and_imperative_deliveries() {
    ...
    let mut conversion_contract = golutra_agent_core::TaskContract::default();
    assert!(
    ...
    let mut layout_contract = golutra_agent_core::TaskContract::default();
