...
use golutra_agent_core::{TaskContract, ToolResultStatus, WorkspaceChangeRequirement};
...
fn verification_issues(record: &golutra_agent_core::VerificationRecord) -> Vec<Value> {
    record
        .assertions
        .iter()
        .filter(|assertion| {
...
fn findings(events: &[RuntimeEvent], task: Option<golutra_agent_core::TaskId>) -> Option<String> {
    let events = events
        .iter()
...
mod tests {
    ...
    fn requested_cancellation_is_an_observation_without_hiding_child_failure() {
        ...
        let task = golutra_agent_core::TaskId::new();
        ...
        different_execution.structured_facts["child_task_id"] =
            json!(golutra_agent_core::TaskId::new());
        assert_eq!(
    ...
    async fn cancellation_observation_survives_reconnect_only_for_matching_execution() {
        ...
        output.structured_facts["child_task_id"] = json!(golutra_agent_core::TaskId::new());
        assert_eq!(
    ...
    fn original_findings_survive_synthetic_failure_and_prior_tasks() {
        ...
        assert_eq!(
            findings(&events, Some(golutra_agent_core::TaskId::new())),
            None
        );
    }
