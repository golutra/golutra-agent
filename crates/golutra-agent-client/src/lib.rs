...
use fs2::FileExt;
use golutra_agent_config::{ProviderConfigPaths, load_provider_runtime_env_from_paths};
pub(crate) use golutra_agent_context::estimate_message_tokens;
#[cfg(test)]
...
#[cfg(test)]
use golutra_agent_eval::EvaluationRunner;
use golutra_agent_eval::{
...
use golutra_agent_evolution::{EvolutionError, EvolutionStore};
use golutra_agent_llm::{
    ConfiguredProvider, LlmProvider, ProviderError, ProviderMessage, protocol_capabilities,
};
use golutra_agent_mcp::McpToolBackend;
use golutra_agent_memory::{MemoryError, MemoryFeedbackKind, MemoryScope, MemoryStore};
use golutra_agent_plugin::PluginStore;
use golutra_agent_policy::{WorkspacePolicy, approval_resource_matches};
...
pub const APP_SERVER_ATTACHMENT_HEADER: &str = "x-golutra-agent-attachment";
pub const APP_SERVER_ACTOR_HEADER: &str = "x-golutra-agent-actor-id";
pub const APP_SERVER_ATTACHMENT_ACTOR_PREFIX: &str = "app-attachment-";
pub const APP_SERVER_PROTOCOL_HEADER: &str = "x-golutra-agent-protocol-version";
pub const APP_SERVER_TRANSPORT_TOKEN_ENV: &str = "GOLUTRA_AGENT_TRANSPORT_TOKEN";
pub const RUNTIME_RELEASE_ID_ENV: &str = "GOLUTRA_AGENT_RELEASE_ID";
...
pub use golutra_agent_protocol::{
...
struct HostedTaskEvaluation<'a> {
    ...
    verification: Option<golutra_agent_core::VerificationRecord>,
    tool_reports: &'a [golutra_agent_tools::ToolExecutionReport],
...
impl RuntimeHost {
    ...
    pub async fn open_persisted_run(run_root: impl AsRef<Path>) -> Result<Arc<Self>, ClientError> {
        ...
        if manifest.format != "golutra-agent-run-bundle"
    ...
    async fn record_orphaned_task_recovery(
        ...
        let (event_type, status) = match record.disposition {
            golutra_agent_core::TaskRecoveryDisposition::Interrupted => {
                (RuntimeEventType::TaskInterrupted, TaskStatus::Interrupted)
            }
            golutra_agent_core::TaskRecoveryDisposition::Uncertain => {
    ...
    async fn record_tool_completed_bundle(
        ...
        evidence: &[golutra_agent_core::EvidenceRecord],
    ) -> Result<(), ClientError> {
        let _writer = self.execution.event_writer.lock().await;
        let causal_before = self.execution.causal_ledger.lock().await.clone();
        let event = match self.prepare_canonical_event(event).await {
    ...
    async fn append_rollout_event(&self, event: &RuntimeEvent) -> Result<(), ClientError> {
        ...
        let durable_boundary = matches!(
            ...
            golutra_agent_protocol::RuntimeEventClass::Control
