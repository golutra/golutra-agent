...
use golutra_agent_context::estimate_tokens;
use golutra_agent_core::{Actor, ActorKind, CommandId, SessionId, TaskStatus};
use golutra_agent_evolution::{
...
impl RuntimeHost {
    ...
    async fn execute_generated_task(
        ...
        generated_task: golutra_agent_eval::GeneratedTask,
    ...
    async fn isolated_fixture_host(
        ...
        RuntimeHost::from_store(super::RuntimeHostBootstrap {
            ...
            default_thread_id: golutra_agent_core::ThreadId::new(),
    ...
    async fn record_skill_event(
        ...
        action: &str,
        record: &golutra_agent_evolution::SkillLifecycleRecord,
    ) -> Result<(), ClientError> {
        self.record_event(host_event(
            self.next_sequence_no(),
            session_id,
