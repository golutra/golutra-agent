...
use golutra_agent_config::{
    ...
    builtin_oauth_methods_for_provider, golutra_agent_home, load_provider_runtime_env,
...
use golutra_agent_eval::{ExternalEvaluationRecord, external_evaluation_result_digest};
...
use golutra_agent_plugin::PluginStore;
use golutra_agent_project_service::{
...
const CLI_ACTOR_ID: &str = "golutra-agent-cli";

mod interactive_entry;
mod mcp_server;

#[derive(Debug, Parser)]
#[command(name = "golutra-agent")]
#[command(version)]
#[command(about = "Simple coding agent for local, observable workspace tasks")]
#[command(
    after_help = "Interactive: golutra-agent [--cwd DIR] [--yolo] [--resume THREAD_ID]\nRequires the sibling golutra-agent-tui native executable; no Node.js/npm required."
)]
struct Cli {
...
enum ProviderCommand {
    ...
    Login {
        ...
        #[arg(long)]
        model: Option<String>,
        #[arg(long, default_value = "GOLUTRA_AGENT_PROVIDER_API_KEY")]
        api_key_env: String,
        #[arg(long)]
    ...
    let command_result: miette::Result<()> = async {
        ...
        Command::Export {
            ...
            let receipt = DebugExportCoordinator::new(&transport)
                .export(DebugExportRequest {
                    selection: golutra_agent_protocol::SessionWindowRequest {
                        anchor_thread_id,
                        range,
                    },
                    destination,
                ...
                update_provider_settings_verified(
                    ...
                    move |user_settings| {
                        if scope == ProviderConfigScope::Workspace {
                            return Err(golutra_agent_config::ConfigError::Validation(
        ...
        Command::Eval {
            ...
            EvalCommand::RecordBenchmark { file } => {
                ...
                let run: golutra_agent_eval::BenchmarkRun = serde_json::from_str(&content)
        ...
        Command::Code { command: code } => {
            ...
            let paths = golutra_agent_client::RuntimePaths::for_cwd(cwd)
                .map_err(|error| miette::miette!("{error}"))?;
            let indexer = golutra_agent_code_intelligence::CodeIntelligence::new(&paths.cwd)
                .map_err(|error| miette::miette!("{error}"))?;
            let store = golutra_agent_code_intelligence::CodeIndexStore::new(&paths.code_index_file);
            match code {
                CodeCommand::Index => {
                    let graph = indexer
                        .build()
                ...
                CodeCommand::Symbols { query, limit } => {
                    ...
                    println!(
                        ...
                        serde_json::to_string_pretty(
                            &golutra_agent_code_intelligence::CodeIntelligence::query_symbols(
                                &graph, &query, limit,
                            ),
                        )
                        .unwrap_or_default()
                ...
                CodeCommand::References { symbol, limit } => {
                    ...
                    println!(
                        "{}",
                        serde_json::to_string_pretty(
                            &golutra_agent_code_intelligence::CodeIntelligence::query_references(
                                &graph, &symbol, limit,
                            ),
                        )
                        .unwrap_or_default()
...
fn load_or_build_code_graph(
    indexer: &golutra_agent_code_intelligence::CodeIntelligence,
    store: &golutra_agent_code_intelligence::CodeIndexStore,
) -> miette::Result<golutra_agent_code_intelligence::CodeGraph> {
...
fn provider_env_for_cli() -> miette::Result<golutra_agent_config::ProviderRuntimeEnv> {
    load_provider_runtime_env().map_err(|error| miette::miette!("{error}"))
}

fn provider_onboarding_for_cli() -> miette::Result<golutra_agent_config::ProviderOnboardingState> {
...
fn command(
    session_id: golutra_agent_core::SessionId,
...
async fn run_exec(
        ...
        loop {
            let next_event = tokio::select! {
                ...
                signal = tokio::signal::ctrl_c() => {
                    signal.map_err(|error| golutra_agent_client::ClientError::TaskExecution(
                        format!("failed to listen for Ctrl+C: {error}"),
                    ))?;
                    if interrupt_requested {
                        return Err(golutra_agent_client::ClientError::TaskExecution(
                            "exec interrupted while waiting for runtime abort".to_owned(),
                        ));
                    }
                    let ack = handle.interrupt().await?;
                    if !ack.accepted {
                        return Err(golutra_agent_client::ClientError::TaskExecution(format!(
            ...
            if json_output {
                ...
                std::io::stdout().flush().map_err(|error| {
                    golutra_agent_client::ClientError::Io(format!(
            ...
            if let Some(approval_id) = approval_id_from_exec_event(&event) {
                let approve = resolve_exec_approval(&approval_id, approval_mode)
                    .await
                    .map_err(|error| {
                        golutra_agent_client::ClientError::TaskExecution(error.to_string())
                    })?;
    ...
    let checkpoint_result = match initial_checkpoint {
        Some(checkpoint) => match checkpoint.await {
            Ok(result) => result,
            Err(error) => Err(golutra_agent_client::ClientError::TaskExecution(format!(
    ...
    let export_result = if let Some(destination) = run_dir {
        ...
        match checkpoint_result {
            ...
            Err(checkpoint_error) => match terminal_export {
                ...
                Err(export_error) => {
                    Err(golutra_agent_client::ClientError::TaskExecution(format!(
...
fn approval_id_from_exec_event(event: &AgentStreamEvent) -> Option<String> {
    ...
    (item.kind == golutra_agent_protocol::AgentItemKind::Approval)
        .then(|| {
            item.data
...
fn run_plugin_command(command: &PluginCommand) -> miette::Result<()> {
    let home = golutra_agent_home().map_err(|error| miette::miette!("{error}"))?;
...
async fn run_project_service_command(
    ...
    let state_root = golutra_agent_client::RuntimePaths::for_cwd(cwd)
        .map_err(|error| miette::miette!("{error}"))?
        .state_dir;
    let manager =
