...
fn run_directory_implies_ephemeral_exec_and_keeps_legacy_alias() {
    ...
    assert!(matches!(
        ...
        Command::Exec(ExecArgs {
            ...
            run_dir: Some(path),
            ..
        }) if path == std::path::Path::new("/tmp/golutra-agent-run")
    ));
...
fn yolo_parses_for_embedded_daemon_connect_and_resume_exec() {
    for arguments in [
        vec!["golutra-agent", "exec", "--yolo", "modify files"],
        vec![
            "golutra-agent",
            "--daemon",
            "exec",
...
fn approval_mode_accepts_compatibility_aliases() {
    for (value, expected) in [
        ...
        let cli =
            Cli::try_parse_from(["golutra-agent", "exec", "--approval-mode", value, "inspect"])
...
fn plugin_commands_parse_explicit_review_and_enable_steps() {
    let review =
        Cli::try_parse_from(["golutra-agent", "plugin", "review", "fixture", "revision-1"])
    ...
    let enable =
        Cli::try_parse_from(["golutra-agent", "plugin", "enable", "fixture", "revision-1"])
...
fn export_command_requires_destination_and_accepts_anchor_range() {
    ...
    assert!(matches!(
        ...
        Command::Export {
            ...
            destination,
            thread_id: Some(_),
        } if range == "+50" && destination == std::path::Path::new("/tmp/golutra-agent-export")
    ));
}
...
fn evaluation_ingest_accepts_an_explicit_artifact_base() {
    let cli = Cli::try_parse_from([
        ...
        "/tmp/golutra-agent-run/terminal-bench-evaluation.json",
    ])
    ...
    assert!(matches!(
        ...
        } if file == std::path::Path::new("/tmp/golutra-agent-run/terminal-bench-evaluation.json")
