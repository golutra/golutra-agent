...
use golutra_agent_core::{EvidenceId, PolicyId};
...
async fn removed_web_search_is_unregistered_even_with_network_access() {
    ...
    for tool_name in ["web_search", "golutra_agent_web_search"] {
        let result = executor
            .execute(
...
async fn registry_contains_p0_tools() {
    ...
    assert!(registry.contract("golutra_agent_web_search").is_none());
...
fn ten_short_child_findings_survive_budget_without_repeated_governance_metadata() {
    let results: Vec<_> = (0..10)
        .map(|index| {
            json!({
        "child_session_id":golutra_agent_core::SessionId::new(), "summary":"subagent completed",
        "content":format!("fact-{index}"), "facts":{
            "child_task_id":golutra_agent_core::TaskId::new(), "child_status":"completed",
...
fn ten_child_wait_results_keep_handles_and_states_under_the_model_budget() {
    let ids: Vec<_> = (0..10)
        .map(|_| golutra_agent_core::SessionId::new().to_string())
        .collect();
...
async fn process_cancellation_terminates_descendants_and_drains_pipes() {
    ...
    let output = tokio::time::timeout(
        ...
        run_process(
            ...
            DEFAULT_TIMEOUT_MS,
            cancellation,
            &golutra_agent_sandbox::SystemSandbox::detect(),
            golutra_agent_sandbox::WorkspaceAccess::ReadWrite,
        ),
    )
    .await
...
async fn process_timeout_interrupts_a_blocked_stdin_write() {
    ...
    let sandbox = golutra_agent_sandbox::SystemSandbox::detect();
    ...
    let output = tokio::time::timeout(
        ...
        run_process_with_progress(
            ProcessExecutionRequest {
                ...
                cancellation: CancellationToken::new(),
                sandbox: &sandbox,
                workspace_access: golutra_agent_sandbox::WorkspaceAccess::ReadWrite,
                allow_network: false,
                stdin: Some(&input),
                isolated_home: false,
            },
...
async fn process_stdin_failure_preserves_the_child_exit_and_output() {
    ...
    let sandbox = golutra_agent_sandbox::SystemSandbox::process_only();
