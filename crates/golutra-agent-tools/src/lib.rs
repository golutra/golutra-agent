...
impl ToolRuntime {
    ...
    fn resolve_verifier_cwd(
        ...
        if (self.policy.mode() != golutra_agent_policy::WorkspacePolicyMode::Unrestricted
    ...
    async fn symbol_search(
        ...
        let result =
            golutra_agent_code_intelligence::CodeIntelligence::query_symbols(&graph, &query, limit);
    ...
    async fn find_references(
        ...
        let limit = bounded_query_limit(&request.arguments);
        let graph = self.build_code_graph(cancellation).await?;
        let result = golutra_agent_code_intelligence::CodeIntelligence::query_references(
            &graph,
            &symbol_name,
            limit,
        );
    ...
    async fn build_code_graph(
        &self,
        cancellation: CancellationToken,
    ) -> Result<golutra_agent_code_intelligence::CodeGraph, ToolError> {
        ...
        let graph = tokio::task::spawn_blocking(move || {
            ...
            let _permit = permit;
            golutra_agent_code_intelligence::CodeIntelligence::new(workspace_root)?.build()
        })
        .await
        .map_err(|error| ToolError::Execution(error.to_string()))?
        .map_err(|error| ToolError::Execution(error.to_string()))?;
    ...
    async fn shell(
        ...
        let command_line = CommandLine::parse_for_execution(
            &command,
            self.policy.mode() == golutra_agent_policy::WorkspacePolicyMode::Unrestricted,
        )?;
...
fn file_content_verification_report(
    ...
    if let Some(evidence) = result.evidence.first_mut() {
        ...
        evidence.verifier = "golutra-agent-tools/file-content-contract".to_owned();
...
fn path_verification_report(
    ...
    if let Some(evidence) = result.evidence.first_mut() {
        evidence.claim = summary.to_owned();
        evidence.verifier = "golutra-agent-tools/path-contract".to_owned();
        evidence.limitations =
...
fn report(
    ...
    let evidence = EvidenceRecord {
        evidence_id: golutra_agent_core::EvidenceId::new(),
        ...
        verifier: "golutra-agent-tools".to_owned(),
        confidence: 0.8,
...
fn shell_command_for_request(arguments: &Value) -> Result<String, ToolError> {
    ...
    if let Some(command) = command {
        ...
        let compatible = match golutra_agent_policy::parse_shell_command_with_input(&command) {
