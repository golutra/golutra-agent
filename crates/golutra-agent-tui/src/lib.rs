use golutra_agent_auth::{CredentialRef, OAuthFlow};
...
pub use golutra_agent_protocol::{DebugProjection, RuntimeEvent, UserProjection};
...
fn parse_auth_login(tokens: &[String]) -> SlashInput {
    ...
    let mut model = None;
    let mut api_key_env = "GOLUTRA_AGENT_PROVIDER_API_KEY".to_owned();
...
#[cfg(test)]
mod tests {
    use golutra_agent_core::{EventId, SessionId};
    ...
    fn slash_parser_accepts_resume_and_auth_login() {
        ...
        assert_eq!(
            parse_slash_input(
                "/auth login --base-url api.golutra.cn --model qwen --api-key-env GOLUTRA_AGENT_KEY --scope user"
            ),
            SlashInput::Command(SlashCommand::Auth(SlashAuthCommand::Login(Box::new(
                OpenAiCompatibleLogin {
                    ...
                    model: "qwen".to_owned(),
                    api_key_env: "GOLUTRA_AGENT_KEY".to_owned(),
                    api_key: None,
