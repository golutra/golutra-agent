use golutra_agent_auth::{CredentialSource, OpenAiDeviceAuthorizationDescriptor};
use golutra_agent_config::ProviderSettings;
use golutra_agent_llm::ProviderReasoningEffort;
use golutra_agent_protocol::{RuntimeEventType, VisibleStep};
...
fn resume_accepts_an_arbitrary_utf8_argument_as_a_string() {
    ...
    let args = Args::try_parse_from(["golutra-agent-tui", "--resume", resume_key])
...
fn yolo_is_global_for_every_tui_entrypoint() {
    for arguments in [
        &["golutra-agent-tui", "--yolo"][..],
        &["golutra-agent-tui", "--daemon", "--yolo"][..],
        &[
            "golutra-agent-tui",
            "--connect",
        ...
        &["golutra-agent-tui", "inspect", "--embedded", "--yolo"][..],
        &[
            "golutra-agent-tui",
            "driver",
            "--embedded",
...
fn full_tool_profile_is_an_explicit_global_tui_option() {
    for arguments in [
        &["golutra-agent-tui", "--tool-profile", "full"][..],
        &[
            "golutra-agent-tui",
            "inspect",
            "--embedded",
...
fn strict_execution_mode_is_an_explicit_global_tui_option() {
    for arguments in [
        &["golutra-agent-tui", "--execution-mode", "strict"][..],
        &[
            "golutra-agent-tui",
            "inspect",
            "--embedded",
...
async fn remote_transport_attaches_to_the_real_app_server_and_resolves_a_session() {
    ...
    let previous_token = std::env::var("GOLUTRA_AGENT_TRANSPORT_TOKEN").ok();
    unsafe {
        std::env::set_var("GOLUTRA_AGENT_HOME", home.path());
        std::env::set_var("GOLUTRA_AGENT_TRANSPORT_TOKEN", token);
    }
    apply_auth_mock().expect("install app-server provider");
    let server = tokio::spawn(golutra_agent_app_server::run(address));
    ...
    let _ = server.await;
    match previous_home {
        Some(value) => unsafe { std::env::set_var("GOLUTRA_AGENT_HOME", value) },
        None => unsafe { std::env::remove_var("GOLUTRA_AGENT_HOME") },
    }
    match previous_token {
        Some(value) => unsafe { std::env::set_var("GOLUTRA_AGENT_TRANSPORT_TOKEN", value) },
        None => unsafe { std::env::remove_var("GOLUTRA_AGENT_TRANSPORT_TOKEN") },
    }
}
...
async fn auth_dialog_mock_choice_persists_global_provider() {
    ...
    assert!(!dir.path().join(".golutra-agent/provider.json").exists());
    match previous_home {
        Some(value) => unsafe {
            std::env::set_var("GOLUTRA_AGENT_HOME", value);
        },
...
async fn structured_question_free_text_uses_unicode_composer_editing() {
    ...
    app.question_dialog = Some(QuestionDialogState::new(UserQuestionRequest {
        ...
        questions: vec![golutra_agent_core::UserQuestionPrompt {
            id: "format".to_owned(),
            header: "Output".to_owned(),
            question: "Choose an output format".to_owned(),
            mode: golutra_agent_core::UserQuestionMode::Single,
            options: vec![
                golutra_agent_core::UserQuestionOption {
                    id: "json".to_owned(),
                    label: "JSON".to_owned(),
                    description: None,
                },
...
fn sending_a_prompt_archives_it_before_the_reply_is_ready() {
    ...
    app.projection = Some(UserProjection {
        session_id,
        task_id: Some(task_id),
        status: golutra_agent_core::TaskStatus::Completed,
        visible_steps: Vec::new(),
        pending_approval: None,
        final_message: None,
        residual_risks: Vec::new(),
...
fn coalescing_completed_tools_does_not_rebuild_archived_conversation() {
    ...
    let first_read = golutra_agent_core::ToolCallId::new();
    let second_read = golutra_agent_core::ToolCallId::new();
...
fn local_status_anchor_prevents_tool_recoalescing_during_reflow() {
    ...
    for (n, path) in [(1, "first.txt"), (2, "second.txt")] {
        let mut event = transcript_event(
            ...
            RuntimeEventType::ToolCompleted,
            json!({
                "envelope": {"tool_call_id": golutra_agent_core::ToolCallId::new(), "tool_name":"read_file",
...
fn activity_rate_is_stable_between_status_samples() {
    let session_id = SessionId::new();
    let task_id = TaskId::new();
    let turn_id = golutra_agent_core::TurnId::new();
...
fn runtime_modal_temporarily_replaces_search_surface_without_losing_query() {
    ...
    app.approval_dialog = Some(ApprovalDialogState::new(ApprovalRequest {
        approval_id: golutra_agent_core::ApprovalId::new(),
        task_id: TaskId::new(),
        turn_id: TurnId::new(),
        tool_call_id: golutra_agent_core::ToolCallId::new(),
...
async fn auth_review_custom_provider_uses_entered_env_key() {
    ...
    assert!(
        !review
            .preview_json
            .contains("\"api_key_env\": \"GOLUTRA_AGENT_PROVIDER_API_KEY\"")
    );
}
...
async fn slash_auth_login_persists_native_anthropic_protocol_without_probe() {
    ...
    let login = OpenAiCompatibleLogin {
        ...
        api_key_env: "GOLUTRA_AGENT_PROVIDER_API_KEY".to_owned(),
...
fn auth_advanced_custom_headers_parse_without_persisting_literal_secrets() {
    let headers = parse_dialog_custom_headers(
        "X-Client=golutra; X-Api-Key=@GOLUTRA_AGENT_PROVIDER_HEADER_KEY",
    )
...
fn new_idle_session_has_empty_transcript() {
    ...
    app.projection = Some(UserProjection {
        session_id: app.session_id,
        task_id: None,
        status: golutra_agent_core::TaskStatus::Idle,
        visible_steps: Vec::new(),
        pending_approval: None,
        final_message: None,
        residual_risks: Vec::new(),
...
fn developer_panel_exposes_governance_without_leaking_into_normal_view() {
    ...
    let verification = golutra_agent_core::VerificationRecord {
        ...
        checks: vec![golutra_agent_core::VerificationCheck {
            kind: golutra_agent_core::VerificationCheckKind::ObjectiveValidation,
        ...
        evidence_refs: Vec::new(),
        result: golutra_agent_core::VerificationResult::Pass,
    ...
    let debug_projection = golutra_agent_protocol::DebugProjection {
        session_id,
        task_id: Some(task_id),
        events: events.clone(),
        event_window: golutra_agent_protocol::DebugEventWindow {
            start_cursor: Some(1),
            end_cursor: Some(2),
    ...
    let completed = tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            let projection = transport
                ...
                .await
                .expect("projection");
            if golutra_agent_client::projection_status(&projection)
...
fn file_changes_are_compact_in_normal_mode_and_detailed_in_developer_mode() {
    ...
    app.set_debug_mode(true);
    app.developer_projection = Some(golutra_agent_protocol::DebugProjection {
        session_id,
        task_id: Some(task_id),
        events: vec![event],
        event_window: golutra_agent_protocol::DebugEventWindow {
...
) -> golutra_agent_protocol::DebugProjection {
    golutra_agent_protocol::DebugProjection {
        session_id,
        task_id,
        event_window: golutra_agent_protocol::DebugEventWindow {
...
async fn malformed_preferences_can_be_repaired_from_the_settings_surface() {
    ...
    unsafe {
        match previous_home {
            Some(previous_home) => std::env::set_var("GOLUTRA_AGENT_HOME", previous_home),
