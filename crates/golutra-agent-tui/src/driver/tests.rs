...
async fn rejected_mouse_activation_does_not_change_local_selection() {
    ...
    let response = driver
        .handle(DriverRequest::InputMouse {
            event: golutra_agent_protocol::DriverMouseEvent {
                kind: golutra_agent_protocol::DriverMouseKind::LeftClick,
...
async fn snapshot_render_does_not_mutate_the_active_pane_layout() {
    ...
    driver.app.developer_projection = Some(golutra_agent_protocol::DebugProjection {
        session_id,
        task_id: None,
