...
#[cfg(unix)]
use fs2::FileExt;
use golutra_agent_client::{RuntimeExecutionOptions, RuntimeTransport};
...
pub(crate) async fn run_inspect_command(args: &Args, command: InspectArgs) -> miette::Result<()> {
    ...
    let mut driver = TuiDriver::launch(
        ...
        DriverLaunchOptions {
            ...
            debug: args.debug,
            yolo: args.yolo,
            execution_mode: args.execution_mode.unwrap_or_default(),
            tool_profile: args.tool_profile.unwrap_or_default(),
            width: command.width,
...
async fn driver_transport(
    ...
    transport.map_err(|error| {
        let hint = if embedded || args.connect.is_some() {
            ...
            "; start golutra-agent-app-server or use --embedded for an isolated process".to_owned()
        };
...
async fn run_socket_driver(
    ...
    eprintln!("golutra-agent-tui driver ready: {}", socket_path.display());
        ...
        tokio::select! {
            accepted = listener.accept() => {
                ...
                if let Err(error) = authenticate_socket_peer(&stream) {
                    ...
                    eprintln!(
                        "golutra-agent-tui driver rejected a socket peer: {}",
                ...
                if let Err(error) = serve_protocol(driver, reader, writer, heartbeat, idle_timeout).await {
                    eprintln!(
                        "golutra-agent-tui driver connection closed with an error: {}",
                        bounded_error(&error.to_string())
                    );
                }
                if driver.closed {
            ...
            _ = ticker.tick() => {
                if let Err(error) = sync_driver(driver).await {
                    let message = bounded_error(&error.to_string());
                    if last_sync_error.as_deref() != Some(message.as_str()) {
                        eprintln!("golutra-agent-tui driver sync failed: {message}");
                        last_sync_error = Some(message);
                    }
                } else {
                    last_sync_error = None;
