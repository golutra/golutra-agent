...
pub(crate) fn help_lines(topic: HelpTopic, keymap: KeymapMode, context: &str) -> Vec<String> {
    match topic {
        ...
        HelpTopic::Navigation => vec![
            "Mouse wheel/drag native terminal scrolling and text selection".to_owned(),
            ...
            "Ctrl+O           open/close the latest tool details".to_owned(),
            "Left/Right       previous/next tool inside details".to_owned(),
            "Esc              return from tool details to inline chat".to_owned(),
            "Alt+C / Ctrl+C   copy saved tool details through OSC52".to_owned(),
            "/, Enter, n      search saved tool output and find next match".to_owned(),
        ],
