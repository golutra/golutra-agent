...
impl RuntimeControls {
    pub(crate) fn discover(current_model: &str, yolo: bool) -> (Self, Vec<ProviderChoice>) {
        let settings = golutra_agent_config::ProviderConfigPaths::global()
            .ok()
            .and_then(|paths| golutra_agent_config::load_provider_settings(&paths).ok());
...
#[cfg(test)]
mod tests {
    use golutra_agent_config::ProviderProfile;
