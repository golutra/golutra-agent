...
pub(crate) fn draw_question_dialog(
    ...
    let mode = match question.mode {
        golutra_agent_core::UserQuestionMode::Single => "select one or enter another answer",
        golutra_agent_core::UserQuestionMode::Multiple => {
            "select one or more, or enter another answer"
        }
    };
    let mut lines = vec![
        Line::from(vec![
    ...
    for (index, option) in question.options.iter().enumerate() {
        ...
        let marker = match (app.preferences.screen_reader, question.mode) {
            (true, golutra_agent_core::UserQuestionMode::Single) => {
                if selected {
                    "selected"
                } else {
                    "option"
            ...
            (true, golutra_agent_core::UserQuestionMode::Multiple) => {
                if selected {
                    "checked"
                } else {
                    "unchecked"
            ...
            (false, golutra_agent_core::UserQuestionMode::Single) => {
                if selected {
                    "(*)"
                } else {
                    "( )"
            ...
            (false, golutra_agent_core::UserQuestionMode::Multiple) => {
                if selected {
                    "[x]"
                } else {
                    "[ ]"
...
pub(crate) fn footer_status_detail(app: &TuiApp) -> String {
    ...
    if app.status_message.trim().is_empty() {
        return match app.projection.as_ref().map(|projection| projection.status) {
            Some(golutra_agent_core::TaskStatus::Idle) | None => "new session".to_owned(),
            _ => String::new(),
        };
    }
    if matches!(
        app.projection.as_ref().map(|projection| projection.status),
        Some(golutra_agent_core::TaskStatus::Running)
...
pub(crate) fn status_chip(app: &TuiApp) -> &'static str {
    ...
    match app.projection.as_ref().map(|projection| projection.status) {
        Some(golutra_agent_core::TaskStatus::Running) => "running",
        Some(golutra_agent_core::TaskStatus::WaitingApproval) => "waiting approval",
        Some(golutra_agent_core::TaskStatus::WaitingAuthentication) => "auth required",
        Some(golutra_agent_core::TaskStatus::Completed) => "complete",
        Some(golutra_agent_core::TaskStatus::Failed) => "failed",
        Some(golutra_agent_core::TaskStatus::Blocked) => "blocked",
        Some(golutra_agent_core::TaskStatus::Cancelled) => "cancelled",
        Some(golutra_agent_core::TaskStatus::Interrupted) => "interrupted",
        Some(golutra_agent_core::TaskStatus::Uncertain) => "uncertain",
        Some(golutra_agent_core::TaskStatus::Aborting) => "aborting",
        Some(golutra_agent_core::TaskStatus::Paused) => "paused",
        Some(golutra_agent_core::TaskStatus::Pausing) => "pausing",
        Some(golutra_agent_core::TaskStatus::Partial) => "partial",
        Some(golutra_agent_core::TaskStatus::Idle) | None => "ready",
    }
}
...
pub(crate) fn status_color(app: &TuiApp) -> Color {
    ...
    match app.projection.as_ref().map(|projection| projection.status) {
        Some(golutra_agent_core::TaskStatus::Running) => palette.accent,
        Some(golutra_agent_core::TaskStatus::Completed) => palette.success,
        Some(golutra_agent_core::TaskStatus::Failed)
        | Some(golutra_agent_core::TaskStatus::Blocked)
        | Some(golutra_agent_core::TaskStatus::Cancelled)
        | Some(golutra_agent_core::TaskStatus::Interrupted)
        | Some(golutra_agent_core::TaskStatus::Uncertain) => palette.error,
        Some(golutra_agent_core::TaskStatus::WaitingApproval)
        | Some(golutra_agent_core::TaskStatus::WaitingAuthentication)
        | Some(golutra_agent_core::TaskStatus::Aborting)
        | Some(golutra_agent_core::TaskStatus::Pausing)
        | Some(golutra_agent_core::TaskStatus::Partial) => palette.warning,
        Some(golutra_agent_core::TaskStatus::Paused) => palette.secondary,
        Some(golutra_agent_core::TaskStatus::Idle) | None => palette.muted,
    }
}
...
pub(crate) fn has_active_task(app: &TuiApp) -> bool {
    matches!(
        ...
        Some(golutra_agent_core::TaskStatus::Running)
            ...
            | Some(golutra_agent_core::TaskStatus::Aborting)
            | Some(golutra_agent_core::TaskStatus::Pausing)
            | Some(golutra_agent_core::TaskStatus::Paused)
    )
}
