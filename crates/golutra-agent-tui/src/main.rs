...
static TUI_ACTOR_ID: LazyLock<String> = LazyLock::new(|| {
    format!(
        "golutra-agent-tui-{}-{}",
        std::process::id(),
        Uuid::now_v7()
...
#[derive(Debug, Parser)]
#[command(name = "golutra-agent-tui")]
#[command(version)]
...
fn parse_runtime_reasoning_effort(
    value: &str,
) -> Option<golutra_agent_llm::ProviderReasoningEffort> {
    match value.trim().to_ascii_lowercase().as_str() {
        "low" => Some(golutra_agent_llm::ProviderReasoningEffort::Low),
    ...
    if let Some(dialog) = &mut app.question_dialog {
        if key.code == KeyCode::Char('s') && key.modifiers.contains(KeyModifiers::CONTROL) {
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if dialog.current_question().mode
                        == golutra_agent_core::UserQuestionMode::Single
                    {
