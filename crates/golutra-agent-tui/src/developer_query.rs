...
use golutra_agent_client::{RuntimeClient, RuntimeTransport};
use golutra_agent_core::{ActorKind, QueryId, SessionId, TaskId};
...
pub(crate) fn replace_debug_event_history_with_window(
    ...
    mut events: Vec<golutra_agent_protocol::RuntimeEvent>,
...
fn refresh_debug_completeness(projection: &mut DebugProjection) {
    ...
    projection.trace_complete = projection.missing_sections.is_empty()
        ...
        && projection.post_task_jobs.iter().all(|job| {
            matches!(
                job.status,
                golutra_agent_core::PostTaskJobStatus::Succeeded
                    | golutra_agent_core::PostTaskJobStatus::Failed
                    | golutra_agent_core::PostTaskJobStatus::Cancelled
            )
        });
}
