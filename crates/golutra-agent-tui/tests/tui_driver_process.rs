...
use golutra_agent_client::{RuntimeClient, RuntimeExecutionOptions, RuntimeTransport};
...
use golutra_agent_llm::{ProviderGenerationConfig, ProviderProtocol};
use golutra_agent_protocol::{
    EventFilter, RuntimeQuery, RuntimeQueryKind, TuiFrame, UserProjection,
};
...
impl HomeEnv {
    fn set(path: &Path) -> Self {
        let previous = std::env::var_os("GOLUTRA_AGENT_HOME");
        // These process tests serialize environment changes with ENV_LOCK.
        unsafe { std::env::set_var("GOLUTRA_AGENT_HOME", path) };
        Self { previous }
    }
}
...
impl Drop for HomeEnv {
    fn drop(&mut self) {
        match self.previous.take() {
            Some(previous) => unsafe { std::env::set_var("GOLUTRA_AGENT_HOME", previous) },
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "requires explicit GOLUTRA_AGENT_TUI_DRIVER_LIVE provider credentials"]
async fn live_provider_driver_smoke_is_isolated_and_opt_in() {
    const ENABLE_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE";
    const API_KEY_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_API_KEY";
    const BASE_URL_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_BASE_URL";
    const MODEL_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_MODEL";
    const PROTOCOL_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_PROTOCOL";
    const MAX_CALLS_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_MAX_CALLS";
    const MAX_OUTPUT_TOKENS_ENV: &str = "GOLUTRA_AGENT_TUI_DRIVER_LIVE_MAX_OUTPUT_TOKENS";
    const EXPECTED_RESPONSE: &str = "GOLUTRA_AGENT_DRIVER_LIVE_OK";
    const EXPECTED_CJK: &str = "中文验收通过";
    const SYNTHETIC_SECRET: &str = "sk-golutra-agent-live-redaction-marker-1234567890";
...
fn tui_command(home: &Path, cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_golutra-agent-tui"));
    command
        .env("GOLUTRA_AGENT_HOME", home)
        .arg("--cwd")
        .arg(cwd)
        .current_dir(cwd);
    command
...
fn start_daemon() -> tokio::task::JoinHandle<miette::Result<()>> {
    tokio::spawn(golutra_agent_app_server::run(
