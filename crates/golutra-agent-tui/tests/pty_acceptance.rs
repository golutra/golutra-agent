...
impl PtyHarness {
    ...
    fn spawn_with_shell_lines(
        ...
        command.arg("-c");
        command.arg("i=0; while [ \"$i\" -lt \"$GOLUTRA_AGENT_PTY_SHELL_LINES\" ]; do printf 'BUILD_LOG\\n'; i=$((i + 1)); done; printf 'SHELL_HISTORY_MARKER\\nSTART_COMMAND_MARKER cargo run -p golutra-agent-tui -- --yolo\\n'; exec \"$@\"");
        command.env("GOLUTRA_AGENT_PTY_SHELL_LINES", shell_lines.to_string());
        command.arg("golutra-agent-pty");
        command.arg(env!("CARGO_BIN_EXE_golutra-agent-tui"));
        command.arg("--cwd");
        command.arg(cwd);
        ...
        command.env("GOLUTRA_AGENT_HOME", home);
        if live {
            command.env("GOLUTRA_AGENT_PROVIDER_MODE", "live");
            command.env("GOLUTRA_AGENT_PTY_TEST_KEY", "isolated-pty-test-key");
        } else {
