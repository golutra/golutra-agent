...
impl ProjectServiceAdapter for TmuxAdapter {
    ...
    fn validate_target(
        ...
        let expected = backend_identifier("golutra-agent", context, false);
    ...
    fn resolve_target(
        ...
        Ok(BackendTarget::Tmux {
            session_name: backend_identifier("golutra-agent", context, false),
        })
    }
...
impl ProjectServiceAdapter for DockerComposeAdapter {
    ...
    fn validate_target(
        ...
        let expected = backend_identifier("golutra-agent", context, true);
    ...
    fn resolve_target(
        ...
        Ok(BackendTarget::DockerCompose {
            project_name: backend_identifier("golutra-agent", context, true),
            compose_file,
...
impl ProjectServiceAdapter for SystemdUserAdapter {
    ...
    fn validate_target(
        ...
        let expected = format!(
            "{}.service",
            backend_identifier("golutra-agent", context, true)
        );
        if unit_name == &expected {
            Ok(())
        } else {
            Err(invalid_descriptor_target(format!(
...
fn backend_identifier(prefix: &str, context: &BackendContext<'_>, lowercase: bool) -> String {
    let mut service = context
        ...
        .collect::<String>();
    // Agent 前缀较长，为后端 63 字符限制预留工作区与服务摘要空间。
    let needs_disambiguation = context.service_name.len() > 18
...
mod tests {
    ...
    fn descriptor_ownership_requires_backend_and_exact_target_match() {
        ...
        assert!(!descriptor_owns_target(
            ...
            &BackendTarget::SystemdUser {
                unit_name: "golutra-agent-web.service".to_owned(),
            },
        ));

        let docker = BackendTarget::DockerCompose {
            project_name: "golutra-agent-project".to_owned(),
    ...
    fn legacy_descriptor_without_phase_is_registered() {
        ...
        let descriptor = serde_json::json!({
            ...
            "target": {
                "backend": "tmux",
                "session_name": "golutra-agent-web"
            }
        });
    ...
    fn backend_identifiers_are_stable_and_backend_safe() {
        ...
        let identifier = backend_identifier("golutra-agent", &context, true);
        assert_eq!(
            identifier,
            format!("golutra-agent-{}-web_dev_1", &fingerprint[..12])
        );
        assert!(identifier.len() <= 63);

        let long = BackendContext {
            service_name: "very-long-service-name-with-Uppercase",
            ..context
        };
        assert!(backend_identifier("golutra-agent", &long, true).len() <= 63);
        // 不允许以历史名称认领外部产品或旧安装的后台资源。
        let legacy = BackendTarget::Tmux {
            session_name: backend_identifier("golutra", &context, false),
        };
        assert!(TmuxAdapter.validate_target(&context, &legacy).is_err());
        let legacy = BackendTarget::SystemdUser {
            unit_name: format!("{}.service", backend_identifier("golutra", &context, true)),
        };
        assert!(
            SystemdUserAdapter
                .validate_target(&context, &legacy)
                .is_err()
        );
        let legacy = BackendTarget::DockerCompose {
        ...
        assert_ne!(
            backend_identifier("golutra-agent", &dotted, true),
            backend_identifier("golutra-agent", &dashed, true)
        );
        ...
        assert_ne!(
            backend_identifier("golutra-agent", &uppercase, true),
            backend_identifier("golutra-agent", &lowercase, true)
        );
    }
    ...
    fn sanitized_backend_environment_allowlists_live_runtime_values() {
        ...
        unsafe {
            std::env::set_var("XDG_RUNTIME_DIR", "/run/user/golutra-agent-test");
            std::env::set_var(
                "DBUS_SESSION_BUS_ADDRESS",
                "unix:path=/run/user/golutra-agent-test/bus",
            );
            ...
            std::env::set_var("HOME", "/home/golutra-agent-test");
        ...
        assert_eq!(
            ...
            Some(std::ffi::OsStr::new("/run/user/golutra-agent-test"))
        );
        assert_eq!(
            values
        ...
        assert_eq!(
            values
                ...
                .map(std::ffi::OsString::as_os_str),
            Some(std::ffi::OsStr::new("/home/golutra-agent-test"))
        );
    ...
    async fn missing_backend_is_reported_without_runtime_ownership() {
        ...
        let error = run_backend_command(
            ProjectServiceBackendKind::Tmux,
            "definitely-not-a-golutra-agent-backend",
            &[],
            workspace.path(),
        )
        .await
