...
use golutra_agent_core::{SessionId, SideEffectType, ToolCallId, ToolResultStatus, TurnId};
use golutra_agent_plugin::{
...
async fn unrestricted_hosts_execute_plugins_without_an_os_sandbox() {
    ...
    let request = request("mcp__fixture__echo", json!({"text": "hello"}));
    let policy = executor.evaluate(&request).expect("policy");
    assert_eq!(policy.decision, golutra_agent_core::PolicyDecision::Allow);
