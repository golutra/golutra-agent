...
fn shell_cli_environment_and_process_waiting_in_isolated_runtime() {
    ...
    for (environment_policy, github) in [
        ...
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("HOME", home.path())
            .env("GOLUTRA_SHELL_ENVIRONMENT_POLICY", environment_policy)
            ...
            .env("GOLUTRA_TRANSPORT_TOKEN", "fixture-internal")
            .env("GOLUTRA_PROVIDER_API_KEY", "fixture-internal")
            .env("GOLUTRA_PROVIDER_CUSTOM_HEADERS", "fixture-internal")
            .env("GOLUTRA_CUSTOM_PROVIDER_API_KEY_TEST", "fixture-internal")
            .env("GOLUTRA_SHELL_TEST_CHILD", "1")
...
async fn shell_cli_child() {
    if std::env::var("GOLUTRA_SHELL_TEST_CHILD").as_deref() != Ok("1") {
        return;
    }
    ...
    fs::write(workspace.path().join("cli.sh"), concat!(
        ...
        "test -z \"${GOLUTRA_TRANSPORT_TOKEN+x}${GOLUTRA_PROVIDER_API_KEY+x}${GOLUTRA_PROVIDER_CUSTOM_HEADERS+x}${GOLUTRA_CUSTOM_PROVIDER_API_KEY_TEST+x}\"\n",
        "if test \"${1-}\" = wait; then\n",
        "  printf x >> launches\n",
        "  read -r value\n",
        "  test \"$value\" = finish\n",
