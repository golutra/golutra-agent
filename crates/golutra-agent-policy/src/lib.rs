    ...
    fn references_sensitive_path(&self, value: &str) -> bool {
        path_like_components(value).any(|component| {
            matches_sensitive_component(component, ".git")
                // 桌面数据仍是外部敏感目录，改名不能降低原有保护。
                || matches_sensitive_component(component, ".golutra")
                || matches_sensitive_component(component, ".golutra-agent")
                || self
                    .sensitive_path_fragments
                    .iter()
...
fn has_internal_runtime_component(path: &Path) -> bool {
    path.components().any(|component| {
        matches!(
            component,
            std::path::Component::Normal(value) if matches!(value.to_str(), Some(".git" | ".golutra" | ".golutra-agent"))
        )
    })
}
...
mod tests {
    ...
    fn unrestricted_mode_allows_outside_sensitive_paths_and_shell_commands() {
        ...
        for command in [
            "cat ~/.ssh/id_ed25519",
            "rm -rf /",
            "printf changed > /tmp/golutra-agent-yolo-test",
            "bash -lc 'cat .git/config; touch /tmp/golutra-agent-yolo-test'",
        ] {
    ...
    fn blocks_internal_runtime_and_git_paths_without_blocking_github_files() {
        ...
        fs::create_dir_all(workspace.path().join(".golutra-agent")).expect("runtime directory");
        ...
        fs::write(
            workspace.path().join(".golutra-agent/runtime.sqlite"),
            "state",
        )
        ...
        assert_eq!(
            policy
                .evaluate_path("write_file", ".golutra-agent/runtime.sqlite", true)
                .decision,
            PolicyDecision::Block
        );
        assert_eq!(
    ...
    fn only_preapproves_inert_shell_commands_without_a_sandbox() {
        ...
        for command in [
            "cargo test -p golutra-agent-core",
            "git status --short",
            "rg 'two words' crates",
            "npm test",
        ] {
