...
use chrono::Utc;
use golutra_agent_core::TraceView;
use golutra_agent_protocol::{
    MINIMUM_RUNTIME_PROTOCOL_VERSION, RUNTIME_PROTOCOL_VERSION, TaskTracePage,
};
use golutra_agent_release::{
...
impl EvolutionSupervisor {
    ...
    fn evaluate_input(
        ...
        let campaign = EvaluationCampaign {
            ...
            evaluator_version: "golutra-agent-supervisor-evaluator-v1".to_owned(),
    ...
    pub async fn bootstrap_stable_release(
        ...
        if !bootstrap_build.gate_accepted {
            let failed_checks = report
                .checks
                .iter()
                .filter(|check| check.status != golutra_agent_release::BuildStatus::Pass)
...
pub fn observation_from_trace(
    ...
    if page
        .verification
        .as_ref()
        .is_some_and(|record| record.result != golutra_agent_core::VerificationResult::Pass)
    {
...
fn task_trace_event_digest(
    events: &[golutra_agent_protocol::RuntimeEvent],
...
fn event_is_failure(event: &golutra_agent_protocol::RuntimeEvent) -> bool {
    match event.event_type {
        golutra_agent_protocol::RuntimeEventType::TaskAborted
        | golutra_agent_protocol::RuntimeEventType::TaskInterrupted
        | golutra_agent_protocol::RuntimeEventType::TaskUncertain
        | golutra_agent_protocol::RuntimeEventType::ProviderAuthFailed => true,
        golutra_agent_protocol::RuntimeEventType::PolicyEvaluated => event
            .payload
            .get("record")
            .and_then(|record| record.get("decision"))
            .and_then(serde_json::Value::as_str)
            .is_some_and(|decision| matches!(decision, "deny" | "block")),
        golutra_agent_protocol::RuntimeEventType::TaskCompleted => event
            .payload
            .get("status")
            .and_then(serde_json::Value::as_str)
...
mod tests {
    ...
    fn setup() -> (EvolutionSupervisor, tempfile::TempDir, tempfile::TempDir) {
        ...
        fs::create_dir_all(source.path().join("crates/golutra-agent-runtime/src"))
            .expect("stable source directory");
        fs::write(
            source
                .path()
                .join("crates/golutra-agent-runtime/src/lib.rs"),
            "stable runtime",
        )
    ...
    fn candidate_worktree(
        ...
        fs::write(
            worktree.join("crates/golutra-agent-runtime/src/lib.rs"),
            runtime_source,
        )
    ...
    fn proposal(
        ...
        CandidateProposal {
            ...
            patch_digest: digest,
            target_paths: vec!["crates/golutra-agent-runtime/src/lib.rs".to_owned()],
    ...
    fn trusted_report(
        supervisor: &EvolutionSupervisor,
        candidate: &EvolutionCandidate,
    ) -> golutra_agent_release::BuildReport {
        ...
        let binary_artifacts = [
            ("golutra-agent", b"fixture cli".as_slice()),
            (
                "golutra-agent-eval-worker",
                b"fixture eval worker".as_slice(),
            ),
        ]
        .into_iter()
        .map(|(name, bytes)| {
            fs::write(artifact_root.join(name), bytes).expect("fixture binary");
            golutra_agent_release::BuildArtifact {
        ...
        golutra_agent_release::BuildReport {
            ...
            checks: vec![golutra_agent_release::BuildCheck {
                name: "workspace-gates".to_owned(),
                command: vec!["cargo test --workspace".to_owned()],
                status: golutra_agent_release::BuildStatus::Pass,
                exit_code: Some(0),
                duration_ms: 1,
    ...
    fn record_trusted_evaluation_build(
        supervisor: &EvolutionSupervisor,
        candidate: &EvolutionCandidate,
        report: golutra_agent_release::BuildReport,
    ) {
    ...
    fn failed_bootstrap_build_remains_queryable_and_verifiable() {
        let (supervisor, _root, _releases) = setup();
        let report = golutra_agent_release::BuildReport {
            builder_version: "test-builder".to_owned(),
            ...
            checks: vec![golutra_agent_release::BuildCheck {
                name: "test".to_owned(),
                command: vec!["cargo test --workspace".to_owned()],
                status: golutra_agent_release::BuildStatus::Fail,
                exit_code: Some(101),
                duration_ms: 1,
    ...
    fn candidate_control_plane_and_exact_holdout_feedback_are_blocked() {
        ...
        blocked.target_paths = vec!["crates/golutra-agent-eval/src/lib.rs".to_owned()];
        ...
        fs::create_dir_all(disguised.join("crates/golutra-agent-supervisor/src"))
            .expect("sealed source directory");
        fs::write(
            disguised.join("crates/golutra-agent-supervisor/src/lib.rs"),
    ...
    async fn trusted_build_rejects_source_changed_after_candidate_freeze() {
        ...
        let source = worktree.join("crates/golutra-agent-runtime/src/lib.rs");
        let candidate = supervisor
            .register_candidate(CandidateProposal {
