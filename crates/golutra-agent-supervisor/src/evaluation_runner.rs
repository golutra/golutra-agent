...
use base64::Engine;
use golutra_agent_core::VerificationResult;
use golutra_agent_protocol::{RuntimeEvaluationWorkerRequest, RuntimeEvaluationWorkerResponse};
use golutra_agent_release::{BuildArtifact, BuildReport, TrustedBuilder};
use golutra_agent_sandbox::{SandboxRequest, SystemSandbox, WorkspaceAccess};
use serde::Serialize;
...
const EVALUATION_WORKER_BINARY: &str = "golutra-agent-eval-worker";
...
struct RuntimeExecutionEvidence {
    ...
    trace: Option<golutra_agent_protocol::TaskTracePage>,
...
fn validate_relative_path(value: &str) -> Result<(), SupervisorError> {
    ...
    if value.trim().is_empty()
        ...
        || path.components().any(|component| {
            matches!(component, Component::Normal(value) if matches!(value.to_str(), Some(".git" | ".golutra" | ".golutra-agent" | "target" | "node_modules")))
        })
    {
        return Err(SupervisorError::Invalid(format!(
