...
const POLICY_ENV: &str = "GOLUTRA_SHELL_ENVIRONMENT_POLICY";
...
pub fn is_internal_environment_variable(name: &str) -> bool {
    INTERNAL_ENVIRONMENT_VARIABLES
        ...
        || name
            .to_ascii_uppercase()
            .starts_with("GOLUTRA_CUSTOM_PROVIDER_API_KEY_")
}
...
mod tests {
    ...
    fn environment(policy: Option<&str>) -> Result<BTreeMap<OsString, OsString>, SandboxError> {
        let mut variables = [
            ...
            ("GOLUTRA_TRANSPORT_TOKEN", "fixture-internal"),
            ("golutra_provider_api_key", "fixture-internal"),
            ("GOLUTRA_PROVIDER_CUSTOM_HEADERS", "fixture-internal"),
            (
                "GOLUTRA_CUSTOM_PROVIDER_API_KEY_EXAMPLE",
                "fixture-internal",
            ),
        ]
    ...
    fn user_filters_cannot_restore_internal_credentials() {
        let inherited = environment(Some(r#"{"include_only":["path","GITHUB_TOKEN","GOLUTRA_TRANSPORT_TOKEN"],"exclude":["github_token"]}"#)).unwrap();
        assert_eq!(
            inherited,
    ...
    fn malformed_policy_fails_without_echoing_values() {
        for policy in [
            ...
            r#"{"set":{"GOLUTRA_TRANSPORT_TOKEN":"fixture-secret"}}"#,
            r#"{"exclude":["*TOKEN*"]}"#,
            "not json",
        ] {
