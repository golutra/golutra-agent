...
use golutra_agent_sandbox::{SandboxLaunch, SandboxRequest, SystemSandbox, WorkspaceAccess};
#[cfg(unix)]
use nix::{
...
impl TrustedBuilder {
    ...
    pub async fn run(
        ...
        Ok(BuildReport {
            builder_version: "golutra-agent-trusted-builder-v1".to_owned(),
            source_digest,
...
mod tests {
    ...
    use golutra_agent_sandbox::{SandboxBackendKind, SandboxLaunch, SystemSandbox};
...
"#,
    ...
    async fn trusted_builder_uses_read_only_source_and_stages_verified_binary() {
        ...
        assert!(!source.path().join("target").exists());
        let staged = artifact_root.join("target/release/golutra-agent");
