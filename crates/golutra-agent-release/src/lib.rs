...
mod tests {
    ...
    fn build_pointer_promote_and_rollback_preserve_immutable_content() {
        let source = tempdir().expect("source");
        fs::create_dir_all(source.path().join("crates/golutra-agent-runtime/src")).expect("mkdir");
        fs::write(
            source
                .path()
    ...
    fn checked_build_copies_verified_binary_and_detects_tampering() {
        ...
        let binary = artifacts.path().join("target/release/golutra-agent");
        ...
        let report = BuildReport {
            ...
            binary_artifacts: vec![BuildArtifact {
                relative_path: "target/release/golutra-agent".to_owned(),
        ...
        let installed = store
            .binary_path(&manifest.release_id, "golutra-agent")
    ...
    fn checked_build_recovers_a_published_bin_before_manifest_update() {
        ...
        let published_binary = published_bin.join("golutra-agent");
        ...
        assert!(
            recovered
                .artifact_digests
                .iter()
                .any(|artifact| artifact.relative_path == "bin/golutra-agent")
        );
        store
            .binary_path(&recovered.release_id, "golutra-agent")
    ...
    fn checked_build_rejects_artifact_paths_that_escape_staging_root() {
        ...
        let outside_binary = outside.path().join("golutra-agent");
