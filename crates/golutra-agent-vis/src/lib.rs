...
use golutra_agent_client::{ClientError, RuntimeClient};
...
pub struct AuditReport {
    ...
    pub verification: Option<golutra_agent_core::VerificationRecord>,
    pub artifact_count: usize,
    pub evidence_count: usize,
    pub event_window: golutra_agent_protocol::DebugEventWindow,
}
...
fn copy_payload_attributes(payload: &Value, attributes: &mut BTreeMap<String, Value>) {
    ...
    if let Some(summary) = payload.get("summary").and_then(Value::as_str) {
        let (redacted, _) = golutra_agent_tools::redact_sensitive_text(summary);
        attributes.insert(
