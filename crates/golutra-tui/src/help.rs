...
pub(crate) fn help_lines(topic: HelpTopic, keymap: KeymapMode, context: &str) -> Vec<String> {
    match topic {
        HelpTopic::Overview => vec![
            ...
            String::new(),
            "Slash pickers such as /resume occupy the full terminal.".to_owned(),
        ...
        HelpTopic::Navigation => vec![
            "Mouse wheel/drag native scrollback and text selection in chat".to_owned(),
            "PageUp/PageDown  scroll the active pane or tool details".to_owned(),
            "Home/End         beginning/end of the active pane".to_owned(),
            "Ctrl+T           switch full transcript/split observations".to_owned(),
            "Ctrl+F           search transcript".to_owned(),
            "Alt+C            copy selection or transcript through OSC52".to_owned(),
            "Alt+R            rich/raw transcript".to_owned(),
            "Ctrl+O           open the latest tool in full-screen details".to_owned(),
            "Left/Right       previous/next tool inside details".to_owned(),
            "Mouse click      select choices or navigate tool details".to_owned(),
            "Back / Esc       return from tool details to inline chat".to_owned(),
            "Mouse wheel      scroll tool details; drag to select text".to_owned(),
        ...
        HelpTopic::WhatsNew => vec![
            ...
            "- Full-screen session picker, help, settings and dashboards".to_owned(),
