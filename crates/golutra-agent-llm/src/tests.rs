...
async fn configured_mock_provider_probe_reports_ready() {
    let result = ConfiguredProvider::probe_from_reader(|key| {
        (key == GOLUTRA_AGENT_PROVIDER_PROTOCOL).then(|| "mock".to_owned())
    })
    .await
    .expect("mock probe");
...
fn multi_edit_schema_keeps_all_replacements_required_for_strict_requests() {
    let contract = golutra_agent_core::ToolContract {
        ...
        output_schema: json!({}),
        error_schema: json!({}),
        side_effect_type: golutra_agent_core::SideEffectType::File,
...
fn configured_provider_resolves_native_anthropic_adapter() {
    let provider = ConfiguredProvider::resolve_from_reader(
        ...
        |key| match key {
            GOLUTRA_AGENT_PROVIDER_PROTOCOL => Some("anthropic".to_owned()),
...
fn provider_protocol_selection_accepts_mode_and_protocol() {
    let from_mode = selected_protocol_from_reader(&|key| match key {
        GOLUTRA_AGENT_PROVIDER_MODE => Some("live".to_owned()),
        _ => None,
    });
    let from_protocol = selected_protocol_from_reader(&|key| match key {
        GOLUTRA_AGENT_PROVIDER_PROTOCOL => Some("openai-compatible".to_owned()),
        GOLUTRA_AGENT_PROVIDER_MODE => Some("mock".to_owned()),
        _ => None,
    });
...
fn native_protocol_redacted_config_prefers_configured_custom_api_key_env() {
    let config = ConfiguredProvider::redacted_from_reader(|key| match key {
        GOLUTRA_AGENT_PROVIDER_PROTOCOL => Some("anthropic".to_owned()),
        GOLUTRA_AGENT_PROVIDER_API_KEY_ENV => {
            Some("GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_TEST".to_owned())
        }
        "GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_TEST" => Some("custom-key".to_owned()),
    ...
    assert_eq!(
        ...
        Some("GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_TEST")
    );
...
fn openai_config_reads_golutra_env_first() {
    let config = OpenAiCompatibleProvider::config_from_env_reader(|key| match key {
        ...
        GOLUTRA_AGENT_PROVIDER_MODEL => Some("golutra-agent-model".to_owned()),
        GOLUTRA_AGENT_PROVIDER_BASE_URL => Some("api.golutra.cn".to_owned()),
        _ => None,
    })
    .expect("config");

    assert_eq!(config.api_key, "golutra-agent-key");
    assert_eq!(config.api_key_env, GOLUTRA_AGENT_PROVIDER_API_KEY);
    assert_eq!(config.model_id, "golutra-agent-model");
...
fn openai_config_prefers_configured_custom_api_key_env() {
    let config = OpenAiCompatibleProvider::config_from_env_reader(|key| match key {
        ...
        "GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_TEST" => Some("custom-key".to_owned()),
        GOLUTRA_AGENT_PROVIDER_API_KEY => Some("generic-key".to_owned()),
        GOLUTRA_AGENT_PROVIDER_MODEL => Some("gpt-5.5".to_owned()),
        GOLUTRA_AGENT_PROVIDER_BASE_URL => Some("https://api.example.com/v1".to_owned()),
        _ => None,
    })
    .expect("config");
...
fn openai_config_reads_generation_config_and_applies_request_body() {
    let config = OpenAiCompatibleProvider::config_from_env_reader(|key| match key {
        ...
        GOLUTRA_AGENT_PROVIDER_MODEL => Some("gpt-5.5".to_owned()),
        GOLUTRA_AGENT_PROVIDER_BASE_URL => Some("api.golutra.cn".to_owned()),
        GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG => Some(
            json!({
...
fn prompt_cache_identity_is_stable_and_session_scoped() {
    ...
    let session_id = golutra_agent_core::SessionId::new();
    first.session_id = Some(session_id);
    first.cache_policy = golutra_agent_core::PromptCachePolicy::Auto;
    ...
    changed.task_id = golutra_agent_core::TaskId::new();
    assert_eq!(first.cache_identity(), changed.cache_identity());

    changed.provider_id = "other-provider".to_owned();
    assert_ne!(first.cache_identity(), changed.cache_identity());

    first.cache_policy = golutra_agent_core::PromptCachePolicy::None;
...
fn provider_affinity_prefers_session_and_falls_back_to_task() {
    ...
    let thread_id = golutra_agent_core::ThreadId::new();
    let parent_session_id = golutra_agent_core::SessionId::new();
...
fn trusted_parent_cache_scopes_use_readable_wire_keys() {
    ...
    compaction_request.cache_policy = golutra_agent_core::PromptCachePolicy::Auto;
    ...
    live_request.cache_policy = golutra_agent_core::PromptCachePolicy::Auto;
...
fn provider_cache_identity_isolated_by_protocol_endpoint() {
    let mut request = request();
    request.session_id = Some(golutra_agent_core::SessionId::new());
    request.cache_policy = golutra_agent_core::PromptCachePolicy::Auto;
...
fn provider_tool_projection_excludes_internal_contract_policy() {
    let mut contract = golutra_agent_core::ToolContract {
        tool_name: "read_file".to_owned(),
        ...
        permission_policy_ref: Some(golutra_agent_core::PolicyId::new()),
    };
...
fn provider_tool_projection_is_canonical_and_schema_changes_invalidate_digest() {
    let contract = |input_schema: Value| golutra_agent_core::ToolContract {
        tool_name: "read_file".to_owned(),
        input_schema,
        output_schema: json!({}),
        error_schema: json!({}),
...
fn provider_tool_projection_uses_the_same_wire_alias_as_transports() {
    ...
    assert_eq!(wire["function"]["name"], "golutra_agent_web_search");
    assert_eq!(
...
fn openai_cache_fields_are_sent_only_for_supported_endpoint() {
    ...
    let session_id = golutra_agent_core::SessionId::new();
    request.session_id = Some(session_id);
    request.cache_policy = golutra_agent_core::PromptCachePolicy::Long;
...
fn openai_compatible_tools_request_leaves_tool_controls_to_provider_defaults() {
    let mut request = request();
    request.tools.push(golutra_agent_core::ToolContract {
        tool_name: "read_file".to_owned(),
        input_schema: json!({"type": "object", "properties": {"path": {"type": "string"}}}),
        output_schema: json!({}),
        error_schema: json!({}),
...
fn provider_cache_profile_gates_compatible_gateway_fields() {
    ...
    assert!(golutra.prompt_cache_key(golutra_agent_core::PromptCachePolicy::Auto));
    assert_eq!(
        golutra.affinity_headers(golutra_agent_core::PromptCachePolicy::Auto),
        vec![
            SESSION_ID_HEADER,
    ...
    assert!(golutra.supports_long_retention(golutra_agent_core::PromptCachePolicy::Long));
    assert_eq!(
    ...
    assert_eq!(
        codex.affinity_headers(golutra_agent_core::PromptCachePolicy::Auto),
    ...
    assert!(
        anthropic
            .affinity_headers(golutra_agent_core::PromptCachePolicy::Auto)
            .is_empty()
    );

    let unknown =
        ProviderCacheProfile::for_provider(ProviderProtocol::OpenAiCompatible, "unknown-gateway");
    assert!(!unknown.prompt_cache_key(golutra_agent_core::PromptCachePolicy::Long));
    assert!(
        unknown
            .affinity_headers(golutra_agent_core::PromptCachePolicy::Long)
            .is_empty()
    );
    assert_eq!(
    ...
    assert!(responses_custom.prompt_cache_key(golutra_agent_core::PromptCachePolicy::Long));
    assert!(responses_custom.supports_long_retention(golutra_agent_core::PromptCachePolicy::Long));
    assert!(
        responses_custom
            .affinity_headers(golutra_agent_core::PromptCachePolicy::None)
            .is_empty()
    );
    assert_eq!(
...
fn provider_cache_profile_keeps_long_retention_explicit() {
    let responses =
        ProviderCacheProfile::for_provider(ProviderProtocol::OpenAiResponses, "custom-responses");
    assert!(responses.supports_long_retention(golutra_agent_core::PromptCachePolicy::Long));
    assert_eq!(
...
fn explicit_cache_capabilities_override_provider_identity() {
    ...
    let config = OpenAiCompatibleProvider::config_from_env_reader(|key| match key {
        "GOLUTRA_AGENT_PROVIDER_API_KEY" => Some("test-key".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("model".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => Some("https://arbitrary.example/v1".to_owned()),
        GOLUTRA_AGENT_PROVIDER_ROUTE_ID => Some("unregistered-gateway".to_owned()),
        GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES => Some(encoded.clone()),
        _ => None,
    })
...
fn provider_route_identity_is_read_without_using_endpoint_host() {
    let config = OpenAiResponsesProvider::config_from_env_reader(|key| match key {
        "GOLUTRA_AGENT_PROVIDER_API_KEY" => Some("test-key".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("gpt-test".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => {
            Some("https://arbitrary-gateway.example/v1".to_owned())
        }
        GOLUTRA_AGENT_PROVIDER_ROUTE_ID => Some("golutra".to_owned()),
        _ => None,
    })
