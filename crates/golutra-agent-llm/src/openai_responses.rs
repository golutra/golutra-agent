...
use golutra_agent_core::{PromptCachePolicy, ProviderContract};
...
impl OpenAiResponsesProvider {
    ...
    fn authenticated_probe_request(
        ...
        headers.insert(
            USER_AGENT,
            HeaderValue::from_str(&format!("golutra-agent/{}", env!("CARGO_PKG_VERSION")))
                .unwrap_or_else(|_| HeaderValue::from_static("golutra-agent")),
        );
        headers.insert("originator", HeaderValue::from_static("golutra-agent"));
...
fn apply_responses_cache_semantics(usage: &mut ProviderUsage) {
    if usage.usage_source != golutra_agent_core::UsageSource::Provider
