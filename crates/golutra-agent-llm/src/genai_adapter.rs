...
mod tests {
    ...
    fn config_debug_never_contains_the_api_key() {
        let provider = GenaiProviderAdapter::from_config(GenaiProviderConfig {
            ...
            api_key_env: crate::GOLUTRA_AGENT_PROVIDER_API_KEY.to_owned(),
    ...
    fn cache_profile_maps_each_protocol_without_leaking_fields() {
        let identity = golutra_agent_core::CacheIdentity {
    ...
    fn genai_request_uses_compact_schema_for_strict_responses_tools() {
        let request = ProviderRequest {
            ...
            tools: vec![golutra_agent_core::ToolContract {
    ...
    fn web_search_is_sent_as_a_custom_function_and_restored_on_return() {
        assert_eq!(wire_tool_name("web_search"), "golutra_agent_web_search");
        assert_eq!(
            restore_wire_tool_name("golutra_agent_web_search"),
            "web_search"
        );
        ...
        let tool = &chat_request.tools.expect("tool list")[0];
        assert_eq!(tool.name.as_ref(), "golutra_agent_web_search");
