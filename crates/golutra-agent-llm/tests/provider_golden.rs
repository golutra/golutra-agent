...
use golutra_agent_auth::{AuthError, CredentialMetadata, CredentialProvider};
...
async fn openai_compatible_custom_headers_are_applied_without_debug_values() {
    ...
    let config = OpenAiCompatibleProvider::config_from_env_reader(|key| match key {
        ...
        "GOLUTRA_AGENT_PROVIDER_API_KEY" => Some(TEST_API_KEY.to_owned()),
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("gpt-golden".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => Some(base_url.clone()),
        "GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS" => Some(
            json!({
                "X-Api-Key": "fake-supplemental-key",
                "X-Client-Name": "golutra-agent-golden",
...
async fn native_genai_affinity_headers_follow_cache_capability_gate() {
    ...
    let config = GenaiProviderAdapter::config_from_env_reader(|key| match key {
        ...
        "GOLUTRA_AGENT_PROVIDER_API_KEY" => Some(TEST_API_KEY.to_owned()),
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("claude-test".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => Some(format!("{base_url}/v1")),
        "GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS" => Some(custom_headers.clone()),
        "GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES" => Some(disabled_capabilities.clone()),
        _ => None,
    })
    ...
    let config = GenaiProviderAdapter::config_from_env_reader(|key| match key {
        ...
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("claude-test".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => Some(format!("{base_url}/v1")),
        "GOLUTRA_AGENT_PROVIDER_AUTH_PROVIDER" => Some("custom-anthropic".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS" => Some(custom_headers.clone()),
        "GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES" => Some(enabled_capabilities.clone()),
        _ => None,
    })
...
async fn openai_responses_internal_affinity_overrides_the_custom_session_header() {
    ...
    let config = OpenAiResponsesProvider::config_from_env_reader(|key| match key {
        "GOLUTRA_AGENT_PROVIDER_PROTOCOL" => Some("openai-responses".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_API_KEY" => Some(TEST_API_KEY.to_owned()),
        "GOLUTRA_AGENT_PROVIDER_MODEL" => Some("gpt-golden".to_owned()),
        "GOLUTRA_AGENT_PROVIDER_BASE_URL" => Some(base_url.clone()),
...
async fn github_copilot_adapter_adds_provider_required_headers() {
    ...
    assert!(
        captured
            .headers
            .get("user-agent")
            .is_some_and(|value| value.starts_with("golutra-agent/"))
    );
}

#[tokio::test]
async fn live_provider_smoke_is_opt_in_and_never_reads_normal_user_credentials() {
    let openai_key = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_COMPATIBLE_API_KEY").ok();
    let openai_base_url = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_COMPATIBLE_BASE_URL").ok();
    let openai_model = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_COMPATIBLE_MODEL").ok();
    ...
    let responses_key = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_RESPONSES_API_KEY").ok();
    let responses_base_url = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_RESPONSES_BASE_URL").ok();
    let responses_model = std::env::var("GOLUTRA_AGENT_LIVE_OPENAI_RESPONSES_MODEL").ok();
    if let (Some(api_key), Some(base_url), Some(model)) =
        (responses_key, responses_base_url, responses_model)
    {
        ...
        let response = OpenAiResponsesProvider::from_config(OpenAiResponsesProviderConfig {
            api_key,
            api_key_env: "GOLUTRA_AGENT_LIVE_OPENAI_RESPONSES_API_KEY".to_owned(),
