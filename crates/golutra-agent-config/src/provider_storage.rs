...
pub fn golutra_agent_home() -> Result<PathBuf, ConfigError> {
    if let Some(value) = std::env::var_os(GOLUTRA_AGENT_HOME_ENV) {
        if value.is_empty() {
            return Err(ConfigError::Validation(
                "GOLUTRA_AGENT_HOME cannot be empty".to_owned(),
            ));
        }
        return Ok(PathBuf::from(value));
    }
    // 保持 npm/Unix 已有 HOME 优先级；原生 Windows 不依赖 shell 注入 HOME。
    ...
    Ok(PathBuf::from(home).join(".golutra-agent"))
}

fn default_user_home(
    home: Option<std::ffi::OsString>,
    user_profile: Option<std::ffi::OsString>,
) -> Result<std::ffi::OsString, ConfigError> {
    home.filter(|value| !value.is_empty())
        .or_else(|| user_profile.filter(|value| !value.is_empty()))
        .ok_or_else(|| {
            ConfigError::Validation("HOME (or USERPROFILE on Windows) is not set".to_owned())
        })
}
...
pub(crate) fn load_provider_settings_unlocked(
    paths: &ProviderConfigPaths,
...
pub(crate) fn load_provider_settings_for_install_unlocked(
    ...
    match load_provider_settings_unlocked(paths) {
    ...
    F: FnOnce(&mut ProviderSettings) -> Result<Vec<SecretMutation>, ConfigError>,
{
    ...
    let loaded = match invalid_settings_policy {
        InvalidProviderSettingsPolicy::Reject => LoadedProviderSettings {
            settings: load_provider_settings_unlocked(paths)
                .map_err(|error| provider_install_error("load", error.to_string()))?,
            rollback_bytes: None,
        },
        InvalidProviderSettingsPolicy::ReplaceJson => {
            load_provider_settings_for_install_unlocked(paths)
                .map_err(|error| provider_install_error("load", error.to_string()))?
        }
    };
    let user_snapshot =
...
#[cfg(test)]
mod home_tests {
    use super::default_user_home;
    use std::ffi::OsString;

    #[test]
    fn native_windows_home_fallback_preserves_existing_home_priority() {
        assert_eq!(
            default_user_home(Some("old".into()), Some("new".into())).unwrap(),
            OsString::from("old")
        );
        assert_eq!(
            default_user_home(None, Some("C:\\Users\\用户".into())).unwrap(),
            OsString::from("C:\\Users\\用户")
        );
        assert_eq!(
            default_user_home(Some("".into()), Some("profile".into())).unwrap(),
            OsString::from("profile")
        );
        assert!(default_user_home(None, Some("".into())).is_err());
    }
}
