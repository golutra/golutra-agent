...
pub const GOLUTRA_AGENT_HOME_ENV: &str = "GOLUTRA_AGENT_HOME";
const PROVIDER_FILE: &str = "provider.json";
const PROVIDER_SETTINGS_VERSION: u32 = 2;
const GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG: &str = "GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG";
const MAX_NON_SECRET_RUNTIME_SETTINGS_BYTES: u64 = 64 * 1024;
pub const CUSTOM_PROVIDER_API_KEY_ENV_PREFIX: &str = "GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_";
...
pub enum ConfigError {
    #[error(
        "config_version_conflict: expected {expected}, actual {actual}; reread before retrying"
    )]
...
impl RuntimeConfig {
    ...
    pub fn p1_default() -> Self {
        Self {
            data_dir: "${GOLUTRA_AGENT_HOME:-~/.golutra-agent}/state".to_owned(),
...
impl ProviderConfigPaths {
    pub fn global() -> Result<Self, ConfigError> {
        Self::from_home(golutra_agent_home()?)
    }
    ...
    pub fn from_home(home: impl AsRef<Path>) -> Result<Self, ConfigError> {
        ...
        if fs::symlink_metadata(home).is_ok_and(|metadata| metadata.file_type().is_symlink()) {
            return Err(ConfigError::Validation(
                "provider home must not be a symbolic link".to_owned(),
            ));
        }
        // 与 runtime 保持同一目录身份；尚未创建的 home 也固定为绝对路径。
        let home = if home.exists() {
            home.canonicalize()
                .map_err(|error| ConfigError::Io(error.to_string()))?
        } else {
            std::path::absolute(home).map_err(|error| ConfigError::Io(error.to_string()))?
        };
        Ok(Self {
...
impl ProviderSettings {
    /// 内容版本用于防止桌面旧快照覆盖并发配置；不包含凭据正文。
    pub fn revision(&self) -> Result<String, ConfigError> {
        revision::content_revision(self)
    }

    /// 必须在现有 verified transaction 的 mutate 闭包内调用，校验与保存共享锁。
    pub fn require_revision(&self, expected: &str) -> Result<(), ConfigError> {
        revision::require_revision(expected, &self.revision()?)
    }
...
impl ProviderInstallPlan {
    pub fn apply(&self, paths: &ProviderConfigPaths) -> Result<(), ConfigError> {
        ...
        let _lock = acquire_provider_settings_lock(path)?;
        let mut settings = load_provider_settings_for_install_unlocked(paths)?.settings;
    ...
    pub fn redacted_values(&self) -> BTreeMap<String, String> {
        self.values
            ...
            .map(|(key, value)| {
                ...
                let redacted = if normalized == GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS
...
pub fn load_provider_runtime_env_for_profile_from_paths(
    ...
    let store = default_secret_store(paths)?;
    let mut settings = load_provider_settings(paths)?;
...
pub fn load_provider_runtime_env_from_paths_with_store(
    ...
    let merged = load_provider_settings(paths)?;
    let auth = AuthService::new(paths.home.clone(), store)
        .map_err(|error| ConfigError::Validation(error.to_string()))?;
...
pub fn provider_onboarding_state_with_store(
    ...
    let user = load_provider_settings(paths)?;
...
pub fn load_non_secret_runtime_settings(
    ...
    let project_dir = workspace_root.join(".golutra-agent");
...
pub fn load_provider_settings(
    ...
    load_provider_settings_unlocked(paths)
}
...
pub fn runtime_env_from_settings(
    ...
    if let Some(profile) = settings.active_profile() {
        ...
        values.insert(GOLUTRA_AGENT_PROVIDER_ROUTE_ID.to_owned(), route_id.clone());
        values.insert(
            "GOLUTRA_AGENT_PROVIDER_PROTOCOL".to_owned(),
            profile.protocol.id().to_owned(),
        );
        values.insert(
            "GOLUTRA_AGENT_PROVIDER_MODE".to_owned(),
        ...
        cache_capabilities
            ...
            .map_err(ConfigError::Validation)?;
        values.insert(
            GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES.to_owned(),
            serde_json::to_string(&cache_capabilities)
                .map_err(|error| ConfigError::Json(error.to_string()))?,
        );
        if let Some(model_id) = &profile.model_id {
            values.insert("GOLUTRA_AGENT_PROVIDER_MODEL".to_owned(), model_id.clone());
        }
        if let Some(base_url) = &profile.base_url {
            values.insert(
                "GOLUTRA_AGENT_PROVIDER_BASE_URL".to_owned(),
                base_url.clone(),
            );
        }
        ...
        {
            values.insert(GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG.to_owned(), value);
        }
        if !profile.custom_headers.is_empty() {
            ...
            values.insert(
                GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS.to_owned(),
        ...
        if let Some(oauth) = &profile.oauth {
            values.insert(
                "GOLUTRA_AGENT_PROVIDER_AUTH_PROVIDER".to_owned(),
        ...
        if let Some(credential_ref) = &profile.credential_ref {
            ...
            values.insert(
                "GOLUTRA_AGENT_PROVIDER_API_KEY_ENV".to_owned(),
                source_label,
            );
            values.insert(
...
/// 桌面编辑 Provider 时使用；在现有事务锁内校验读取版本，探测与回滚语义保持不变。
pub async fn update_provider_settings_at_revision_verified<F>(
    paths: &ProviderConfigPaths,
    workspace_root: impl AsRef<Path>,
...
{
    update_provider_settings_verified(paths, workspace_root, |settings| {
        settings.require_revision(expected_revision)?;
        mutate(settings)
    })
    .await
}
