...
impl IsolatedGolutraHome {
    fn new() -> Self {
        ...
        let previous = std::env::var_os(GOLUTRA_AGENT_HOME_ENV);
        unsafe {
            std::env::set_var(GOLUTRA_AGENT_HOME_ENV, dir.path());
        }
        Self {
            previous,
            _dir: dir,
...
impl Drop for IsolatedGolutraHome {
    fn drop(&mut self) {
        match &self.previous {
            Some(value) => unsafe {
                std::env::set_var(GOLUTRA_AGENT_HOME_ENV, value);
            },
            None => unsafe {
                std::env::remove_var(GOLUTRA_AGENT_HOME_ENV);
            },
        }
    }
}
...
fn oauth_descriptor() -> OAuthProviderDescriptor {
    OAuthProviderDescriptor {
        ...
        flows: vec![golutra_agent_auth::OAuthFlow::BrowserPkce],
    }
}
...
fn empty_golutra_home_is_rejected() {
    ...
    let _home = ScopedEnvVar::set(GOLUTRA_AGENT_HOME_ENV, "");

    let error = golutra_agent_home().expect_err("empty home must be rejected");
...
#[test]
fn legacy_env_map_is_rejected_without_config_or_credential_writes() {
    let dir = tempdir().expect("dir");
  ...
  "env": {"GOLUTRA_AGENT_PROVIDER_API_KEY": "legacy-secret"},
  "profiles": [
    {
      "name": "golutra",
      "protocol": "openai-compatible",
      ...
      "api_key_env": "GOLUTRA_AGENT_PROVIDER_API_KEY",
      "enabled": true
    },
    {
      "name": "backup",
...
"#,
    ...
    let before = fs::read(&paths.user_config).unwrap();
    let error = load_provider_settings(&paths).unwrap_err();
    assert!(
        error
            .to_string()
            .contains("unsupported provider settings version 1")
    );
    assert_eq!(fs::read(&paths.user_config).unwrap(), before);
    assert!(
        !paths
            .home
            .join(golutra_agent_auth::CREDENTIALS_FILE_NAME)
            .exists()
    );
    // 登录入口也不能把旧配置视为可修复 JSON 而覆盖。
    assert!(load_provider_settings_for_install_unlocked(&paths).is_err());
    assert_eq!(fs::read(&paths.user_config).unwrap(), before);
}
...
fn provider_install_plan_accepts_native_live_protocols() {
    ...
    let profile = ProviderProfile {
        ...
        credential_ref: Some(env_credential("GOLUTRA_AGENT_PROVIDER_API_KEY")),
        oauth: None,
        generation_config: None,
        custom_headers: Vec::new(),
        cache_capabilities: Some(golutra_agent_llm::ProviderCacheCapabilities::anthropic()),
        enabled: true,
    };
    let plan = ProviderInstallPlan {
        scope: ProviderConfigScope::User,
...
fn provider_custom_headers_resolve_env_values_without_exposing_them_in_debug() {
    let _home = IsolatedGolutraHome::new();
    let _api_key = ScopedEnvVar::set("GOLUTRA_AGENT_TEST_PROVIDER_KEY", "fake-provider-key");
    let _header = ScopedEnvVar::set("GOLUTRA_AGENT_TEST_HEADER_KEY", "fake-header-secret");
    let paths = ProviderConfigPaths::global().expect("paths");
    let mut profile = ProviderProfile::openai_compatible(
        "custom-headers",
        "https://api.example.com/v1",
        "model-test",
        env_credential("GOLUTRA_AGENT_TEST_PROVIDER_KEY"),
    )
    .expect("profile");
    profile.custom_headers = vec![
        ProviderHeaderConfig {
            ...
            value: ProviderHeaderValue::Literal {
                value: "golutra-agent-test".to_owned(),
            },
        },
        ProviderHeaderConfig {
            name: "X-Api-Key".to_owned(),
            value: ProviderHeaderValue::Environment {
                key: "GOLUTRA_AGENT_TEST_HEADER_KEY".to_owned(),
            },
        },
    ];
    ProviderInstallPlan {
    ...
    let headers: BTreeMap<String, String> = serde_json::from_str(
        &runtime
            .get(GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS)
            .expect("resolved headers"),
    )
    .expect("header JSON");

    assert_eq!(headers["X-Client-Name"], "golutra-agent-test");
    assert_eq!(headers["X-Api-Key"], "fake-header-secret");
    assert_eq!(
        runtime.redacted_values()[GOLUTRA_AGENT_PROVIDER_CUSTOM_HEADERS],
        "<redacted>"
    );
...
fn onboarding_does_not_implicitly_activate_the_first_enabled_profile() {
    ...
    assert!(
        load_provider_runtime_env()
            .expect("runtime env")
            .get("GOLUTRA_AGENT_PROVIDER_PROTOCOL")
            .is_none()
    );
}
...
fn onboarding_accepts_disk_backed_provider_without_exposing_secret() {
    let _home = IsolatedGolutraHome::new();
    let paths = ProviderConfigPaths::global().expect("paths");
    let store = Arc::new(golutra_agent_auth::MemorySecretStore::default());
    let reference = disk_credential();
    store
...
fn onboarding_accepts_key_from_configured_env_var() {
    let _home = IsolatedGolutraHome::new();
    let paths = ProviderConfigPaths::global().expect("paths");
    let _api_key = ScopedEnvVar::set("GOLUTRA_AGENT_PROVIDER_API_KEY", "secret-from-env");
    let profile = ProviderProfile::openai_compatible(
        "golutra",
        "api.golutra.cn",
        "gpt-test",
...
async fn runtime_env_uses_dynamic_secret_provider_instead_of_serializing_key() {
    ...
    assert_eq!(
        runtime_env.get("GOLUTRA_AGENT_PROVIDER_API_KEY"),
        Some("<resolved-credential>".to_owned())
    );
    assert_eq!(
        runtime_env.get(golutra_agent_llm::GOLUTRA_AGENT_PROVIDER_ROUTE_ID),
...
fn runtime_env_exports_declared_cache_capabilities() {
    let profile = ProviderProfile::openai_compatible(
        ...
        "https://api.golutra.cn/v1",
        "gpt-5.5",
        env_credential("GOLUTRA_AGENT_CACHE_CAPABILITY_TEST_KEY"),
    )
    .expect("profile");
    let settings = ProviderSettings {
        version: PROVIDER_SETTINGS_VERSION,
    ...
    let auth = AuthService::new(
        home.path(),
        Arc::new(golutra_agent_auth::MemorySecretStore::default()),
    )
    .expect("auth");

    let runtime = runtime_env_from_settings(&settings, &auth).expect("runtime env");
    let encoded = runtime
        .get(GOLUTRA_AGENT_PROVIDER_CACHE_CAPABILITIES)
...
fn runtime_env_uses_oauth_route_identity_for_cache_capabilities() {
    ...
    assert_eq!(
        runtime.get(GOLUTRA_AGENT_PROVIDER_ROUTE_ID).as_deref(),
        Some("openai-chatgpt")
    );
...
fn runtime_env_keeps_unknown_compatible_gateway_cache_disabled() {
    let mut profile = ProviderProfile::openai_compatible(
        ...
        "model",
        env_credential("GOLUTRA_AGENT_UNKNOWN_GATEWAY_TEST_KEY"),
    )
    .expect("profile");
...
fn runtime_env_includes_generation_config_json() {
    let mut profile = ProviderProfile::openai_compatible(
        ...
        "https://api.golutra.cn/v1",
        "gpt-5.5",
        env_credential("GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_TEST"),
    )
    .expect("profile");
    profile.generation_config = Some(ProviderGenerationConfig {
        enable_thinking: true,
        reasoning_effort: Some(golutra_agent_llm::ProviderReasoningEffort::High),
    ...
    let value = runtime_env
        .get(GOLUTRA_AGENT_PROVIDER_GENERATION_CONFIG)
...
fn redacted_runtime_env_hides_hashed_custom_provider_keys() {
    let key = "GOLUTRA_AGENT_CUSTOM_PROVIDER_API_KEY_OPENAI_COMPATIBLE_EXAMPLE_1234";
    let environment = ProviderRuntimeEnv {
        values: BTreeMap::from([
            (key.to_owned(), "private-value".to_owned()),
            (
                "GOLUTRA_AGENT_PROVIDER_MODEL".to_owned(),
                "model".to_owned(),
            ),
        ]),
        credential: None,
...
async fn verified_provider_install_persists_secret_in_the_disk_store() {
    ...
    let store = golutra_agent_auth::DefaultSecretStore::new(&paths.home).expect("disk store");
    assert_eq!(
        store
            .get(&reference)
...
fn non_secret_runtime_settings_load_project_layer() {
    ...
    let workspace = tempdir().expect("workspace");
    let paths = ProviderConfigPaths::from_home(home.path()).expect("paths");
    fs::create_dir_all(workspace.path().join(".golutra-agent")).expect("project config dir");
    fs::write(
    ...
    fs::write(
        workspace.path().join(".golutra-agent/runtime.json"),
