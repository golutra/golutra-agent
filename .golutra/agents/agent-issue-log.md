...
| --- | --- | --- | --- |
| 2026-07-06 09:16 | TUI 启动沿用 workspace default session/thread，导致普通进入不是新会话；首屏和普通任务 transcript 展示 provider/context/checkpoint 等内部步骤，视觉上像 debug demo；bootstrap 空 thread 还会出现在 session 列表。 | 参考 Codex 将普通启动和 resume 分离：TUI 默认生成新的 ThreadId/SessionId，首个 prompt 才持久化该 thread；`/resume` 仍显式恢复历史；普通 transcript 只显示用户输入、关键工具结果和完成状态；隐藏 ready provider 常态行和 bootstrap placeholder thread。 | `cargo test -p golutra-client`、`cargo test -p golutra-tui`、`cargo fmt --all -- --check`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo test --workspace` 通过；TUI 伪终端 smoke 创建 `tui.txt=ok`，thread list 仅显示真实 TUI 会话。 |
