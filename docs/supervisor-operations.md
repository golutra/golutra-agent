...
`golutra-agent-supervisor` 是独立于普通 RuntimeHost 的控制面。它只消费完整、脱敏的 `TaskTracePage`，在隔离 worktree 接收 internal/external producer 的代码候选，并负责密封评测、可信构建、preview/canary、稳定指针和回滚。

普通 CLI/TUI 不启动 Supervisor，也不展示这些治理状态。当前任务始终由启动它的版本完成；`golutra-agent-launcher` 只在下一次启动时读取新的 stable pointer，不在 active task 中途替换进程。
...
```text
$GOLUTRA_AGENT_HOME/evolution/supervisor/
  state.json
  control.jsonl
...
```text
SUPERVISOR_ARGS="--root $GOLUTRA_AGENT_HOME/evolution/supervisor --releases $GOLUTRA_AGENT_HOME/releases"
```

查看状态和校验控制日志：

```text
golutra-agent-supervisor $SUPERVISOR_ARGS status
golutra-agent-supervisor $SUPERVISOR_ARGS verify-log
```

第一次运行控制面时，先把当前受信源码构建为初始 stable release。该命令只允许执行一次，并要求 Seatbelt/bubblewrap、离线 Cargo、完整测试和 `golutra-agent-eval-worker` 产物全部通过：

```text
golutra-agent-supervisor $SUPERVISOR_ARGS bootstrap /path/to/golutra-agent
```

CLI 的完整 trace JSON 可以直接进入观察器：

```text
golutra-agent --cwd /workspace trace --task-id <task-id> --full --wait-evaluation > task-trace.json
golutra-agent-supervisor $SUPERVISOR_ARGS observe-trace task-trace.json \
  --independent-group <run-id>
```

`golutra-agent-launcher` 会把实际 stable release ID 写入进程环境；RuntimeHost 将该 identity 固定到任务起始事件和 `TaskTracePage`。Supervisor 从 event chain 验证 identity，不接受调用方填写 `--source-version`。只有同一个 source release 上跨独立任务重复的 failure cluster，或该 release 上单次确定性 security/integrity/recovery 缺陷，才会生成 opportunity。单条普通失败只记录 observation；source version 会进入 cluster identity，且 `start-epoch` 只接受与当前 stable pointer 相同的 source release，旧版本证据不能直接驱动新 stable 的代码修改。

启动有限 epoch，并由 producer 生成候选：

```text
golutra-agent-supervisor $SUPERVISOR_ARGS start-epoch <opportunity-id>
golutra-agent-supervisor $SUPERVISOR_ARGS prepare-worktree <epoch-id> <candidate-id>
golutra-agent-supervisor $SUPERVISOR_ARGS produce candidate-request.json \
...
```text
golutra-agent-supervisor $SUPERVISOR_ARGS evaluate evaluation-suite.json
golutra-agent-supervisor $SUPERVISOR_ARGS check <candidate-id>
golutra-agent-supervisor $SUPERVISOR_ARGS build <candidate-id>
golutra-agent-supervisor $SUPERVISOR_ARGS preview <candidate-id>
golutra-agent-supervisor $SUPERVISOR_ARGS canary <candidate-id>
golutra-agent-supervisor $SUPERVISOR_ARGS canary-observation canary-observation.json
golutra-agent-supervisor $SUPERVISOR_ARGS promote <candidate-id> --reason "healthy canary"
```

`evaluation-suite.json` 只包含 case 输入、fixture、partition 和 Supervisor 侧 assertion。assertion、partition、真实 case id 与 sealed 答案都不会传给 worker。每个 case 分别启动 stable release 和 candidate evaluation build 中的 `golutra-agent-eval-worker`；二者使用独立 home/workspace、相同输入、随机目录、无网络 OS sandbox 和 deterministic mock provider。Supervisor 在进程外复核 workspace 文件、完整 TaskTrace、VerificationRecord 以及每个 artifact blob 的 checksum/size，并把含 blob 的配对 evidence 持久化为 `artifact://supervisor-evaluation/...`。缺少 development/security/migration/sealed/fresh 任一 partition 的 suite 会被拒绝。
...
```text
golutra-agent-supervisor $SUPERVISOR_ARGS rollback <candidate-id> --reason "canary health regression"
```

下一次启动稳定版本：

```text
GOLUTRA_AGENT_RELEASE_HOME="$GOLUTRA_AGENT_HOME/releases" golutra-agent-launcher golutra-agent chat
```
