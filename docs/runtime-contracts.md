...
- Embedded、Unix IPC 与 HTTP/SSE transport 使用同一 `SessionCommand`、`RuntimeQuery`、`RuntimeEvent` 语义和 protocol version；加入 `FailureEpisodeRecorded`、diagnosis revision、外部 evidence import 和精确 context attribution 后的当前 runtime protocol 为 v6，旧 reader 不与新事件流协商。用户级 daemon 通过 attachment 路由多个 cwd，Unix socket owner-only，HTTP 仅允许安全 endpoint 并校验 bearer/Host/Origin。SQLite 在 event append 事务内原子分配全局 sequence，host 再按提交顺序 publish；command lease 与 durable ack 负责重试去重，但 command ack 与后续业务事件仍不是一个跨运行时事务。
...
- checkpoint 每 workspace 保留最近 20 个；artifact 维护按 retention/expiry 清理 blob，并保护仍被 lineage、verification 或 rollback 引用的记录。相同 checksum/size 的 artifact blob 与 checkpoint before-image 使用内容寻址硬链接去重；逻辑 record、权限、retention 和 provenance 仍保持独立。
...
- external evaluation 必须验证 source digest、runtime identity、canonical result digest 和 trust。声明的本地 evaluator 文件会复制为不可变、checksummed、owner-only artifact，再生成 `EvidenceRecord`；外部失败到达后必须重新投影 `FailureEpisode -> FailureDiagnosis -> DiagnosticSlice -> ImprovementCandidate`。holdout 只接受 owner trust store 中 Ed25519 key 的有效签名；regression 逐 `case × partition × provider × seed` 要求 baseline/candidate pair，缺 cell 时只能 `NeedsReview`。
