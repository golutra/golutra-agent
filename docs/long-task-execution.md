...
- Nonterminal text continues without a fixed continuation count; malformed terminal states still fail, and an incomplete auxiliary summary is not installed.
...
Implicit contracts do not impose a correction-round limit in either client payload normalization or the Rust harness. A failed candidate feeds the existing structured verification envelope into the same task, with original history, permissions and budgets. Successful plain conversation still takes one provider request. An explicit contract, including a zero-round limit, takes precedence. An omitted or null `max_correction_rounds` means unlimited; an integer is an explicit limit, including zero to disable correction. Explicitly conversational contracts still disable correction.
...
Default execution has no total time, tool-count, consecutive-failure, cumulative-cost, correction-count or no-progress cutoff. Repeated actions receive a progress advisory, not an automatic strategy ban. Explicit governor budgets remain enforceable; zero disables a scalar governor limit. Network waiting is excluded from an explicitly configured correction inactivity clock but counts toward an explicit overall deadline. Process-crash recovery still requires reconciliation of uncertain effects. Child concurrency defaults to ten (configurable), while sequential children have no implicit aggregate token quota or 30-minute deadline.
