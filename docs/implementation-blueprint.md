...
9. `exec --run-dir` 在 turn 开始即写入 `in_progress` checkpoint；raw state 与 owner-only observations 可在进程异常退出后重开。正常终态先以快速路径原子替换 checkpoint，debug export/evaluator overlay 通过显式 bundle refresh 生成；刷新验证旧 event prefix 不变，并以 pending 文件表达未完成收集。
