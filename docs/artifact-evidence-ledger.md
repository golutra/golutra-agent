...
- artifact metadata 位于 SQLite，blob 位于 owner-only `$GOLUTRA_AGENT_HOME/state/artifacts`；写入前统一 redaction，记录 SHA-256、size、created_at、retention policy、expiry 和 blob deletion state。
...
- 普通 TUI 不展示 artifact 原文；developer/debug、`golutra-agent-vis` 和 SDK query 才按需展开审计事实。
